...
//   // 👇 [PERBAIKAN] State untuk Otorisasi CRUD Transaksi 👇
...
//     // 👇 [PERBAIKAN] Cek RBAC sebelum redirect
//     if (!canAccess('transactions.detail')) {
//         Swal.fire("Akses Ditolak", "Anda tidak memiliki izin untuk melihat detail transaksi.", "error");
//         return;
//     }
...
//           {/* 👇 BUNGKUS TOMBOL EXPORT DENGAN RBAC 👇 */}
//           {canAccess('transactions.export') && (
//             <>
//               <button
//                 onClick={exportToPDF}
...
//                     className={`align-top transition border-b group border-gray-50 ${canAccess('transactions.detail') ? 'cursor-pointer hover:bg-emerald-50/20' : 'cursor-default'}`}
//                   >
//                     {/* ORDER DETAILS */}
//                     <td className="py-6 pl-2 w-[15%]">
...
//                           {/* 👇 BUNGKUS TOMBOL APPROVE/REJECT REFUND DENGAN RBAC 👇 */}
...
export default function TransactionPage() {
  ...
  // State untuk Otorisasi CRUD Transaksi
  const [adminUser, setAdminUser] = useState<any>(null);
  const [allowedModules, setAllowedModules] = useState<string[]>([]);
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const data = await res.json();
      const filteredData = data.filter((o: any) => o.status !== "awaiting_payment");
      const mappedData = filteredData.map((o: any) => ({ ...o, isCancelling: false }));
  ...
  const handleRefundAction = async (e: React.MouseEvent, id: number, action: "approve" | "reject") => {
    e.stopPropagation();
    const endpoint = action === "approve" ? "refund-approve" : "refund-reject";
    const confirmText = action === "approve" ? "Setujui permintaan refund ini?" : "Tolak permintaan refund ini?";
    ...
    if (result.isConfirmed) {
      try {
        const token = localStorage.getItem("admin_token");
        const res = await fetch(`${BASE_URL}/api/admin/transactions/${id}/${endpoint}`, {
  ...
  const exportToPDF = () => {
    ...
    const headers = element.querySelectorAll(".export-header");
    headers.forEach((h) => { h.classList.remove("hidden"); h.classList.add("block"); });

    const opt = {
      margin: 0.3, filename: `Transaction_Report_${new Date().toISOString().split("T")[0]}.pdf`,
      image: { type: "jpeg" as const, quality: 0.98 }, 
      html2canvas: { scale: 2, useCORS: true },
    ...
    html2pdf().set(opt).from(element).save().then(() => {
        headers.forEach((h) => { h.classList.add("hidden"); h.classList.remove("block"); });
        noExportElements.forEach((el) => el.classList.remove("hidden"));
    });
  };
  ...
  const exportToExcel = () => {
    ...
    const excelData = paginatedTransactions.map((item, index) => ({
      ...
      [`Points Discount (${curr})`]: convertIDRtoActiveCurrency((item.points_used || 0) * 1000).value,
      [`Grand Total (${curr})`]: convertIDRtoActiveCurrency(getGrandTotal(item)).value,
      "Points Earned": item.status === "completed" ? item.point || 0 : 0,
      "Transaction Status": item.status.replace(/_/g, " ").toUpperCase(),
      "Shipping Status": item.shipping_method === "free" ? "IN-STORE" : (item.shipping_status || "PENDING").toUpperCase(),
    }));

    const worksheet = XLSX.utils.json_to_sheet(excelData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Transactions");
    XLSX.writeFile(workbook, `Transaction_Data_${new Date().toISOString().split("T")[0]}.xlsx`);
  };
              ...
              <tbody className="text-sm text-gray-600">
                {paginatedTransactions.map((trx) => (
                  ...
                  >
                    ...
                    <td className="py-6 pl-2 w-[15%]">
                      <div className="flex flex-col gap-1">
                        ...
                        {trx.status === "pending" &&
                          countdowns[trx.id] !== "Expired" && (
                            <div className="flex items-center gap-1 px-2 py-1 mt-2 text-red-600 rounded-md no-export bg-red-50 w-fit">
                              <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className={`text-[9px] font-bold uppercase tracking-wider ${getPaymentStatusColor(trx.status)}`}>
                              {getPaymentStatusText(trx.status)}
                            </p>
                          </div>
                        </div>
                    ...
                    {/* FINANCIALS MENGGUNAKAN MULTI-CURRENCY CONVERTER */}
                    <td className="py-6 w-[15%] pr-2">
                      <div className="flex flex-col gap-1.5">
                        <div className="flex justify-between text-[10px] text-gray-500">
                          <span>Subtotal:</span>
                          <span>{formatCurrencyDisplay(convertIDRtoActiveCurrency(parseFloat(trx.total_amount)))}</span>
                        </div>
                        <div className="flex justify-between text-[10px] text-gray-500">
                          <span>Shipping:</span>
                          <span>{formatCurrencyDisplay(convertIDRtoActiveCurrency(parseFloat(trx.shipping_cost)))}</span>
                        </div>
                        ...
                        {trx.promo_discount > 0 && (
                          <div className="flex justify-between text-[9px] text-emerald-600 font-bold">
                            ...
                            <span>-{formatCurrencyDisplay(convertIDRtoActiveCurrency(parseFloat(trx.promo_discount)))}</span>
                          </div>
                        )}
                        {trx.points_used > 0 && (
                          <div className="flex justify-between text-[9px] text-yellow-600 font-bold">
