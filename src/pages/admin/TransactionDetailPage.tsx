...
//   // --- EXPORT INVOICE LOGIC ---
//   const generateInvoicePDF = () => {
//     setIsExportingInvoice(true);
    
//     setTimeout(() => {
//       const element = document.getElementById("export-invoice-template");
//       if(!element) {
//           setIsExportingInvoice(false);
//           return Swal.fire("Error", "Elemen Invoice PDF tidak ditemukan", "error");
//       }
...
//             icon: "success",
//             title: "Berhasil",
//             text: "Faktur/Invoice berhasil diunduh.",
//             confirmButtonColor: "#059669",
//             timer: 2000,
...
//           setIsExportingInvoice(false);
//           Swal.fire("Error", "Gagal memproses Invoice PDF", "error");
//         });
//     }, 1000);
//   };
...
//       {(isPreparingPDF || isExportingInvoice) && (
...
//             {/* TEMPLATE: CETAK RESI LOGISTIK */}
//             <div id="print-label-template" className={`bg-white text-black p-8 w-[800px] shrink-0 border-[3px] border-black flex flex-col gap-4 ${isExportingInvoice ? 'hidden' : ''}`}>
//               {transaction && (
//                 <>
...
//             {/* TEMPLATE: EXPORT INVOICE DETAIL TRANSAKSI */}
//             <div id="export-invoice-template" className={`bg-white text-black p-10 w-[800px] shrink-0 font-sans ${isPreparingPDF ? 'hidden' : ''}`}>
//               {transaction && (
//                 <>
//                   {/* Header Invoice */}
//                   <div className="flex items-start justify-between pb-6 mb-6 border-b border-gray-300">
//                     <div>
//                       <h1 className="text-3xl font-black tracking-widest uppercase text-gycora-dark">GYCORA</h1>
//                       <p className="mt-1 text-sm text-gray-500">Official Online Store</p>
//                       <p className="mt-1 text-sm text-gray-500">Surabaya, Indonesia</p>
//                     </div>
//                     <div className="text-right">
//                       <h2 className="mb-2 text-4xl font-black tracking-wider text-gray-800 uppercase">INVOICE</h2>
//                       <p className="text-sm font-bold text-gray-600">Order ID: {transaction.order_id}</p>
//                       <p className="text-sm text-gray-500">Date: {new Date(transaction.created_at).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
//                       <p className="inline-block px-2 py-1 mt-2 text-sm font-bold tracking-widest text-gray-700 uppercase bg-gray-100 rounded">
...
//                     <div className="w-1/2">
//                       <h3 className="pb-2 mb-3 text-xs font-bold tracking-widest text-gray-400 uppercase border-b border-gray-200">Billed To</h3>
//                       <p className="text-lg font-bold text-gray-900">{transaction.user.first_name} {transaction.user.last_name}</p>
//                       <p className="text-sm text-gray-600">{transaction.user.email}</p>
//                       <p className="text-sm text-gray-600">{transaction.user.phone || "-"}</p>
//                     </div>
//                     <div className="w-1/2">
//                       <h3 className="pb-2 mb-3 text-xs font-bold tracking-widest text-gray-400 uppercase border-b border-gray-200">Shipped To</h3>
//                       <p className="text-lg font-bold text-gray-900">{transaction.address?.first_name_address} {transaction.address?.last_name_address}</p>
//                       <p className="mt-1 text-sm leading-relaxed text-gray-600">{transaction.address?.address_location}</p>
//                       <p className="text-sm text-gray-600">{transaction.address?.city}, {transaction.address?.province} - {transaction.address?.postal_code}</p>
//                     </div>
//                   </div>
...
//                       <thead>
//                         <tr className="text-xs tracking-widest text-gray-600 uppercase bg-gray-100">
//                           <th className="px-4 py-3 font-bold rounded-tl-lg">Item Description</th>
...
//                               {item.color && <p className="mt-1 text-xs text-gray-500">Color: {item.color}</p>}
//                               <p className="mt-1 font-mono text-xs text-gray-400">SKU: {item.product.sku}</p>
//                             </td>
//                             <td className="px-4 py-4 font-medium text-center text-gray-900">{item.quantity}</td>
//                             <td className="px-4 py-4 text-sm text-right text-gray-600">{formatPrice(item.price)}</td>
//                             <td className="px-4 py-4 font-bold text-right text-gray-900">{formatPrice(item.quantity * item.price)}</td>
//                           </tr>
//                         ))}
//                       </tbody>
//                     </table>
...
//                     <div className="w-1/2 p-6 border border-gray-100 bg-gray-50 rounded-xl">
//                       <div className="flex justify-between mb-3 text-sm text-gray-600">
//                         <span>Subtotal</span>
//                         <span className="font-medium text-gray-900">{formatPrice(transaction.total_amount)}</span>
//                       </div>
//                       <div className="flex justify-between mb-3 text-sm text-gray-600">
//                         <span>Shipping ({transaction.courier_company?.toUpperCase() || 'Free'})</span>
...
//                         <span className="font-bold tracking-widest text-gray-900 uppercase">Grand Total</span>
//                         <span className="text-2xl font-black text-gycora-dark">{formatPrice(getGrandTotal(transaction))}</span>
//                       </div>
//                     </div>
//                   </div>
...
//                       <h3 className="mb-2 text-xs font-bold tracking-widest text-gray-400 uppercase">Payment Details</h3>
//                       <p className="text-sm font-bold text-gray-900 uppercase">{transaction.payment_method.replace('_', ' ')}</p>
//                       {transaction.payment && <p className="mt-1 font-mono text-xs text-gray-500">Ref ID: {transaction.payment.external_id}</p>}
//                     </div>
//                   )}

//                   <div className="pt-6 mt-12 text-xs text-center text-gray-400 border-t border-gray-200">
//                     <p>Thank you for shopping at Gycora.</p>
...
//           {/* TOMBOL EXPORT INVOICE */}
//           <button
//             onClick={generateInvoicePDF}
//             className="flex items-center gap-2 px-4 py-2 text-xs font-bold text-gray-700 transition bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-50 hover:text-gray-900"
//           >
...
//           {transaction?.shipping_method === 'biteship' && <div className="hidden w-px h-8 bg-gray-200 sm:block"></div>}
...
//                       {/* INDIKATOR WARNA DENGAN LINGKARAN */}
//                       {item.color && (
...
//                         <p 
//                           onClick={() => copyToClipboard(biteshipData?.courier?.waybill_id || transaction.tracking_number || "Pending Allocation", "Nomor Resi")}
//                           className="flex items-center gap-2 font-mono text-sm font-bold text-black transition cursor-pointer hover:text-gycora group"
//                         >
//                           {biteshipData?.courier?.waybill_id || transaction.tracking_number || "Pending Allocation"}
//                           <svg className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
//                         </p>
//                       </div>
//                     </div>
//                   </div>
...
//                   <div 
//                     onClick={() => copyToClipboard(transaction.order_id, "Order ID")}
//                     className="flex items-center justify-between px-3 py-2 font-mono text-xs transition rounded-lg cursor-pointer bg-white/10 hover:bg-white/20 group"
//                   >
//                     {transaction.order_id}
//                     <svg className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
//                   </div>
//                 </div>
//               </div>
//             </div>
...
//   // [BARU] State untuk tombol retry shipping
...
//   // =========================================================
//   // [BARU] FUNGSI RETRY REQUEST PENGIRIMAN BITESHIP
//   // =========================================================
//   const handleRetryShipping = async () => {
//     setIsRetryingShipping(true);
//     try {
//       const token = localStorage.getItem("admin_token");
//       const res = await fetch(`${BASE_URL}/api/admin/transactions/${transaction.id}/retry-shipping`, {
//         method: "POST",
//         headers: {
...
//           confirmButtonColor: "#059669",
//         }).then(() => {
//           // Segarkan data halaman agar resi baru muncul
//           fetchData();
//         });
//       } else {
//         Swal.fire({
//           icon: "warning",
//           title: "Gagal Request Ulang",
//           text: data.message || "Pastikan saldo Biteship Anda mencukupi.",
...
//   const isShippingError = transaction?.shipping_status === 'error' || String(transaction?.tracking_number).includes('ERR');
...
//       rejected: "Pengiriman ditolak oleh kurir.",
//       cancelled: "Pengiriman telah dibatalkan.",
//       error: "Sistem logistik gagal memproses pesanan ini.",
//     };
//     return map[status] || "Pesanan sedang diproses oleh sistem.";
//   }
...
//                         {/* 👇 TOMBOL RETRY SHIPPING JIKA ERROR 👇 */}
//                         {isShippingError && (
//                           <button 
//                             onClick={handleRetryShipping} 
//                             disabled={isRetryingShipping} 
//                             className="px-3 py-1.5 text-[10px] font-bold tracking-widest text-white uppercase transition-colors bg-red-600 rounded-md shadow-sm hover:bg-red-700 disabled:opacity-50"
//                           >
//                             {isRetryingShipping ? "Retrying..." : "Retry Request"}
//                           </button>
//                         )}
...
// import html2pdf from "html2pdf.js";
// import { BASE_URL } from "../../config/api";
// import { useCurrency } from "../../context/CurrencyContext"; // [BARU] Import Context Currency
...
//   // 👇 [PERBAIKAN] Tarik state currency dan exchangeRates
...
//   // State untuk tombol retry shipping
//   const [isRetryingShipping, setIsRetryingShipping] = useState(false);
...
//   // =========================================================
//   // FUNGSI RETRY REQUEST PENGIRIMAN BITESHIP
//   // =========================================================
//   const handleRetryShipping = async () => {
//     setIsRetryingShipping(true);
//     try {
...
//   // 👇 [PERBAIKAN] LOGIKA GROSIR 👇
//   const isWholesaleOrder = transaction?.user?.usertype === 'reseller' && totalQuantity >= 24;
...
//                     <div className="py-4 text-center border-b-2 border-black">
//                       <p className="text-2xl font-black text-black">
//                         Ongkos Kirim: {formatCurrencyDisplay(convertIDRtoActiveCurrency(transaction.shipping_cost))}
//                       </p>
//                       {transaction.promo_discount > 0 && (
//                         <p className="mt-1 text-lg font-bold text-black">
//                           Diskon Promo: - {formatCurrencyDisplay(convertIDRtoActiveCurrency(transaction.promo_discount))}
//                         </p>
//                       )}
//                       <p className="mt-1 text-lg font-bold text-black">
//                         Jenis Layanan - {transaction.courier_type}
...
//                               <div className="flex items-center gap-2">
//                                 <p className="font-bold text-gray-900">{item.product.name}</p>
//                                 {isWholesaleOrder && <span className="px-1.5 py-0.5 text-[8px] font-bold text-white bg-blue-600 rounded">GROSIR</span>}
//                               </div>
//                               {item.color && <p className="mt-1 text-xs text-gray-500">Color: {item.color}</p>}
//                               <p className="mt-1 font-mono text-xs text-gray-400">SKU: {item.product.sku}</p>
//                             </td>
//                             <td className="px-4 py-4 font-medium text-center text-gray-900">{item.quantity}</td>
//                             <td className="px-4 py-4 text-sm text-right text-gray-600">{formatCurrencyDisplay(convertIDRtoActiveCurrency(item.price))}</td>
//                             <td className="px-4 py-4 font-bold text-right text-gray-900">{formatCurrencyDisplay(convertIDRtoActiveCurrency(item.quantity * item.price))}</td>
//                           </tr>
//                         ))}
//                       </tbody>
//                     </table>
...
//                       <div className="flex justify-between mb-3 text-sm text-gray-600">
//                         <span>Subtotal</span>
//                         <span className="font-medium text-gray-900">{formatCurrencyDisplay(convertIDRtoActiveCurrency(transaction.total_amount))}</span>
//                       </div>
//                       <div className="flex justify-between mb-3 text-sm text-gray-600">
//                         <span>Shipping ({transaction.courier_company?.toUpperCase() || 'Free'})</span>
//                         <span className="font-medium text-gray-900">{formatCurrencyDisplay(convertIDRtoActiveCurrency(transaction.shipping_cost))}</span>
//                       </div>
//                       {transaction.promo_discount > 0 && (
//                         <div className="flex justify-between mb-3 text-sm text-emerald-600">
//                           <span>Discount (Promo)</span>
//                           <span>-{formatCurrencyDisplay(convertIDRtoActiveCurrency(transaction.promo_discount))}</span>
//                         </div>
//                       )}
//                       {transaction.points_used > 0 && (
//                         <div className="flex justify-between mb-3 text-sm text-emerald-600">
//                           <span>Discount (Points)</span>
//                           <span>-{formatCurrencyDisplay(convertIDRtoActiveCurrency(transaction.points_used * 1000))}</span>
//                         </div>
//                       )}
//                       <div className="flex justify-between pt-4 mt-4 border-t border-gray-300">
//                         <span className="font-bold tracking-widest text-gray-900 uppercase">Grand Total</span>
//                         <span className="text-2xl font-black text-gycora-dark">{formatCurrencyDisplay(convertIDRtoActiveCurrency(getGrandTotal(transaction)))}</span>
//                       </div>
//                     </div>
//                   </div>
...
//                       <div className="flex items-end justify-between mt-4">
//                         <p className="px-3 py-1 text-sm text-gray-600 rounded-lg bg-gray-50">
//                           {item.quantity} <span className="text-[10px] mx-1">x</span> {formatCurrencyDisplay(convertIDRtoActiveCurrency(item.price))}
//                         </p>
//                         <p className="font-bold text-black">{formatCurrencyDisplay(convertIDRtoActiveCurrency(item.quantity * item.price))}</p>
//                       </div>
//                     </div>
//                   </div>
//                 ))}
...
//                 <div className="flex items-center justify-between text-sm text-gray-600">
//                   <span>Subtotal ({totalQuantity} items)</span>
//                   <span className="font-medium text-gray-900">{formatCurrencyDisplay(convertIDRtoActiveCurrency(parseFloat(transaction.total_amount)))}</span>
//                 </div>
//                 <div className="flex items-center justify-between text-sm text-gray-600">
//                   <span>Shipping Fee</span>
//                   {transaction.shipping_method === 'free' ? (
//                     <span className="font-bold text-emerald-600">Free</span>
//                   ) : (
//                     <span className="font-medium text-gray-900">{formatCurrencyDisplay(convertIDRtoActiveCurrency(parseFloat(transaction.shipping_cost)))}</span>
//                   )}
//                 </div>
...
//                   <div className="flex items-center justify-between text-sm font-medium text-emerald-600">
//                     <span>Promo Code ({transaction.promo_code})</span>
//                     <span>- {formatCurrencyDisplay(convertIDRtoActiveCurrency(parseFloat(transaction.promo_discount)))}</span>
//                   </div>
//                 )}
//                 {transaction.points_used > 0 && (
//                   <div className="flex items-center justify-between text-sm font-medium text-yellow-600">
...
//                     <span className="text-[10px] text-gray-400 italic">Paid by customer</span>
//                   </div>
//                   <span className="text-2xl font-bold text-black">{formatCurrencyDisplay(convertIDRtoActiveCurrency(getGrandTotal(transaction)))}</span>
//                 </div>
//               </div>
...
export default function TransactionDetailPage() {
  ...
  // =========================================================
  // FUNGSI APPROVE MANUAL PESANAN FRAUD
  // =========================================================
  const handleApproveFraud = async () => {
    try {
      const token = localStorage.getItem("admin_token");
      const res = await fetch(`${BASE_URL}/api/admin/transactions/${transaction.id}/approve-fraud`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Accept": "application/json"
      ...
      if (res.ok) {
        Swal.fire("Berhasil", "Transaksi telah disetujui dan dilepaskan dari penahanan (Hold).", "success");
        fetchData();
      } else {
        const err = await res.json();
        Swal.fire("Gagal", err.message || "Gagal melakukan approve manual.", "error");
      }
    } catch (err) {
  ...
  function getNoteFromStatus(status: string) {
    const map: Record<string, string> = {
      ...
      cancelled: "Pengiriman telah dibatalkan.",
      error: "Sistem logistik gagal memproses pesanan ini.",
      on_hold: "Pesanan ini sedang ditinjau manual oleh tim karena peringatan keamanan.",
    };
    return map[status] || "Pesanan sedang diproses oleh sistem.";
  }
  ...
  const getPaymentStatusText = (status: string) =>
    ["completed", "processing", "refund_requested", "refund_approved", "refund_rejected", "on_hold"].includes(status)
      ? "PAID/PENDING"
      : status === "cancelled" ? "CANCELLED" : status === "refunded" ? "REFUNDED" : "UNPAID";
  ...
  const getPaymentStatusColor = (status: string) =>
    ["completed", "processing", "refund_requested", "refund_approved", "refund_rejected"].includes(status)
      ? "text-emerald-600"
      : status === "on_hold" ? "text-red-500" : status === "cancelled" ? "text-red-500" : status === "refunded" ? "text-teal-600" : "text-orange-500";
  ...
  // Safe fallback if fraud properties are undefined on old transactions
  const fraudScore = transaction?.fraud_score ?? 0;
  const fraudFlags = transaction?.fraud_flags ?? [];
    ...
    <div className="relative max-w-6xl min-h-screen px-6 py-12 mx-auto font-sans">
            ...
            <div id="print-label-template" className={`bg-white text-black p-8 w-[800px] shrink-0 border-[3px] border-black flex flex-col gap-4 ${isExportingInvoice ? 'hidden' : ''}`}>
              {transaction && (
                <>
                  ...
                  <div className="flex flex-col items-center py-6 text-center border-b-2 border-black">
                    <img
                      src={`[https://bwipjs-api.metafloor.com/?bcid=code128&text=$](https://bwipjs-api.metafloor.com/?bcid=code128&text=$){biteshipData?.courier?.waybill_id || transaction.tracking_number || transaction.order_id}&scale=4&includetext=false`}
                      className="object-contain h-24 mb-4"
                      crossOrigin="anonymous"
                      alt="Barcode"
                    />
                  ...
                  <div className="flex gap-6 py-6 border-b-2 border-black">
                    <div className="flex flex-col w-1/2 pr-6 border-r-2 border-black">
                      <p className="mb-3 text-sm font-bold text-black">Reference Number:</p>
                      <img
                        src={`[https://bwipjs-api.metafloor.com/?bcid=code128&text=$](https://bwipjs-api.metafloor.com/?bcid=code128&text=$){transaction.order_id}&scale=3&includetext=false`}
                        className="self-start object-contain h-16"
                        crossOrigin="anonymous"
                        alt="Reference Barcode"
                      />
      ...
      <div className="relative z-10 flex flex-col justify-between gap-4 mb-10 sm:flex-row sm:items-center">
        <button
          ...
          className="flex items-center gap-2 text-gray-500 transition-colors w-fit hover:text-gray-900 group"
        >
          <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" className="w-5 h-5 transition-transform group-hover:-translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" />
          </svg>
          <span className="text-sm font-bold tracking-widest uppercase">Back to List</span>
        </button>
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center px-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="flex flex-col w-full max-w-lg overflow-hidden bg-white shadow-2xl rounded-xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-bold text-gray-800">Print Label</h3>
              <button onClick={() => setShowPrintModal(false)} className="text-gray-400 transition hover:text-red-500">
                <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            </div>
      ...
      {isLoading ? (
        ...
        <div className="relative z-10 grid grid-cols-1 gap-8 lg:grid-cols-3 animate-fade-in">
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            {/* ========================================================= */}
            {/* 👇 [BARU] FRAUD DETECTION ENGINE CARD 👇 */}
            {/* ========================================================= */}
            {transaction.fraud_score !== undefined && (
              <div className={`p-6 border shadow-sm rounded-[2rem] ${
                fraudScore >= 80 
                ...
                <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
                  ...
                  <div className="flex items-center gap-5">
                    <div className={`flex flex-col items-center justify-center w-20 h-20 rounded-full border-4 shrink-0 ${
                      fraudScore >= 80 ? "border-red-500 bg-red-100 text-red-700" 
                      : fraudScore >= 30 ? "border-orange-400 bg-orange-100 text-orange-700" 
                      : "border-emerald-500 bg-emerald-100 text-emerald-700"
                    }`}>
                      <span className="text-2xl font-black">{fraudScore}%</span>
                      <span className="text-[9px] font-bold uppercase tracking-widest text-center leading-tight mt-1">Skor Risiko</span>
                    </div>

                    <div>
                      <h3 className={`text-lg font-extrabold flex items-center gap-2 ${
                        fraudScore >= 80 ? "text-red-800" : fraudScore >= 30 ? "text-orange-800" : "text-emerald-800"
                      }`}>
                        {fraudScore >= 80 ? "🚨 POTENSI PENIPUAN TINGGI (FRAUD)" 
                        : fraudScore >= 30 ? "⚠️ PERLU PERHATIAN" 
                        : "✅ TRANSAKSI AMAN"}
                      </h3>
                      
                      {fraudScore >= 80 ? (
                        <p className="mt-1 text-sm font-medium text-red-600">
                          Transaksi ini otomatis <strong>DITAHAN (On Hold)</strong> oleh AI Keamanan. Harap periksa alasan di bawah sebelum menyetujui pesanan.
                        </p>
                      ) : (
                        <p className="mt-1 text-sm font-medium text-emerald-600">Sistem tidak mendeteksi anomali perilaku mencurigakan pada pembeli ini.</p>
                      )}
                    </div>
                  </div>

                  {/* Tombol Approve Manual (Hanya muncul jika status on_hold) */}
                  {transaction.status === "on_hold" && (
                    <div className="shrink-0">
                      <button 
                        onClick={handleApproveFraud}
                        className="flex items-center gap-2 px-6 py-3 text-sm font-bold text-white transition-all bg-red-600 rounded-xl hover:bg-red-700 hover:shadow-lg animate-pulse"
                      >
                ...
                {/* Daftar Pelanggaran (Flags) */}
                {fraudFlags && fraudFlags.length > 0 && (
                  <div className="pt-4 mt-6 border-t border-red-200/50">
                    <p className="mb-3 text-xs font-bold tracking-widest text-gray-500 uppercase">Deteksi Anomali AI:</p>
                    <ul className="space-y-2">
                      {fraudFlags.map((flag: string, index: number) => (
                        <li key={index} className="flex items-start gap-3 p-3 bg-white border shadow-sm rounded-lg border-red-100">
                          <span className="mt-0.5 text-lg">{flag.includes('Foreign') ? '🔴' : flag.includes('Velocity') ? '⚡' : '⚠️'}</span>
            ...
            <div className="bg-white shadow-sm p-8 border border-gray-100 rounded-[2rem]">
              ...
              {transaction.point > 0 && transaction.status === 'completed' && (
                <div className="flex items-center justify-between p-4 mt-4 border rounded-xl bg-gradient-to-r from-emerald-50 to-white border-emerald-100">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center justify-center w-10 h-10 text-white rounded-full shadow-sm bg-emerald-500">
                      <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" className="w-6 h-6" viewBox="0 0 20 20" fill="currentColor">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    </div>
                    <div>
