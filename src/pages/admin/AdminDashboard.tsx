...
// import ABTestWidget from "../../components/admin/ABTestWidget";
// import PredictiveInventoryWidget from "../../components/admin/PredictiveInventoryWidget";
// import CohortAnalysisWidget from "../../components/admin/CohortAnalysisWidget";
...
//   // STATE UNTUK AI BUSINESS ANALYST
//   const [aiInsight, setAiInsight] = useState<string | null>(null);
//   const [isAiLoading, setIsAiLoading] = useState(false);
...
//                     dy={10}
//                   />
//                   {/* 👇 [PERBAIKAN] Tampilan angka Y-Axis utuh dengan width tambahan agar tidak terpotong 👇 */}
//                   <YAxis
//                     axisLine={false}
//                     tickLine={false}
//                     tick={{ fill: "#6b7280", fontSize: 11 }}
...
// const DEFAULT_CHART_COLORS = ["#059669", "#3b82f6", "#f59e0b", "#ef4444", "#8b5cf6", "#ec4899", "#14b8a6", "#6366f1"];
...
//   // 👇 [BARU] STATE UNTUK MAINTENANCE MODE 👇
...
//   // 👇 [BARU] FUNGSI TOGGLE MAINTENANCE 👇
//   const handleToggleMaintenance = async () => {
//     const actionText = isMaintenance ? "Mematikan" : "Menyalakan";
//     const result = await Swal.fire({
//       title: `${actionText} Maintenance Mode?`,
//       text: isMaintenance
//         ? "Sistem akan kembali online dan bisa diakses seluruh dunia."
//         : "Seluruh pelanggan akan melihat layar perbaikan. Hanya perangkat Anda (IP ini) yang bisa mengakses web untuk kebutuhan pengujian (testing).",
//       icon: 'warning',
//       showCancelButton: true,
//       confirmButtonColor: isMaintenance ? '#10b981' : '#ef4444',
...
//       try {
//         const token = localStorage.getItem("admin_token");
//         const res = await fetch(`${BASE_URL}/api/admin/maintenance/toggle`, {
//           method: 'POST',
//           headers: { Authorization: `Bearer ${token}` }
//         });
//         const data = await res.json();
...
//         }
//       } catch (error: any) {
//         Swal.fire("Error", error.message || "Gagal mengubah mode maintenance", "error");
//       }
//     }
//   };
...
//         {payload.map((entry: any, index: number) => (
//           <li key={`item-${index}`} className="flex items-start gap-2 text-[11px] text-gray-600 leading-snug">
//             <span className="w-2 h-2 rounded-full shrink-0 mt-[3px]" style={{ backgroundColor: entry.color }} />
//             <span className="break-words whitespace-normal">{entry.value}</span>
//           </li>
//         ))}
//       </ul>
...
//         {/* 👇 [BARU] TOMBOL MAINTENANCE 👇 */}
//         <button
//           onClick={handleToggleMaintenance}
//           className={`flex items-center gap-3 px-5 py-2.5 rounded-xl text-[10px] md:text-sm font-bold shadow-sm transition-all ${isMaintenance ? 'bg-red-50 text-red-600 border border-red-200 animate-pulse' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}
//         >
//           <div className="relative flex items-center justify-center w-3 h-3">
//             {isMaintenance && <span className="absolute inline-flex w-full h-full bg-red-400 rounded-full opacity-75 animate-ping"></span>}
//             <span className={`relative inline-flex rounded-full w-2.5 h-2.5 ${isMaintenance ? 'bg-red-500' : 'bg-emerald-500'}`}></span>
//           </div>
//           {isMaintenance ? 'MAINTENANCE AKTIF' : 'SYSTEM ONLINE'}
//         </button>
//       </div>
...
//           </div>
//           <p className="mb-1 text-sm font-semibold tracking-wider text-gray-500 uppercase">Total Pendapatan</p>
//           <h3 className="text-2xl font-extrabold text-gray-900 truncate" title={formatRupiah(stats.total_sales)}>{formatRupiah(stats.total_sales)}</h3>
//           <p className={`flex items-center gap-1 mt-2 text-sm font-medium ${stats.sales_growth >= 0 ? "text-emerald-600" : "text-red-500"}`}>
//             {stats.sales_growth >= 0 ? (
//               <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
//             ) : (
...
//           <p className="mb-1 text-sm font-semibold tracking-wider uppercase text-emerald-100">Total Pelanggan</p>
//           <h3 className="text-3xl font-extrabold">{stats.total_users || 0}</h3>
//           <p className="mt-2 text-sm font-medium text-white">+{stats.new_users_growth || 0} pengguna baru bulan ini!</p>
//         </div>
//       </div>

//       {/* WIDGET AI BUSINESS ANALYST */}
//       <div className={`transition-all duration-700 delay-300 ${isDataReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}>
//         <div className="relative p-6 overflow-hidden text-white border border-indigo-800 shadow-xl bg-gradient-to-br from-indigo-900 via-indigo-800 to-purple-900 rounded-2xl group">
//           <div className="absolute top-0 right-0 p-4 transition-transform duration-700 transform translate-x-8 -translate-y-8 opacity-10 group-hover:scale-110">
//             <svg className="w-48 h-48 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zm0 7.5l-10-5v2.5l10 5 10-5V9.5l-10 5zM2 14v2.5l10 5 10-5V14l-10 5-10-5z" /></svg>
//           </div>
...
//                 Gemini AI Engine
//               </div>
//               <h2 className="flex items-center gap-2 text-2xl font-extrabold text-white">AI Business Analyst</h2>
//               <p className="mt-1.5 text-sm text-indigo-200 max-w-xl leading-relaxed">Dapatkan rekomendasi restock pintar dan analisis performa penjualan mingguan secara instan berdasarkan pergerakan data transaksi real-time.</p>
//             </div>

//             <button onClick={generateAiInsight} disabled={isAiLoading} className="px-6 py-3 font-bold text-indigo-900 bg-white rounded-xl shadow-[0_0_20px_rgba(255,255,255,0.3)] hover:shadow-[0_0_25px_rgba(255,255,255,0.5)] transition-all hover:-translate-y-0.5 whitespace-nowrap flex items-center justify-center gap-2 disabled:opacity-70 disabled:hover:translate-y-0 disabled:shadow-none">
//               {isAiLoading ? (
//                 <><svg className="w-5 h-5 text-indigo-600 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Menganalisis Data...</>
//               ) : (
//                 <><svg className="w-5 h-5 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg> Generate Laporan</>
//               )}
//             </button>
//           </div>
...
//           {aiInsight && (
//             <div className="p-6 mt-8 border shadow-inner bg-white/10 backdrop-blur-md rounded-xl border-white/20 animate-fade-in-up">
//               <div className="text-sm leading-relaxed prose text-indigo-50 prose-invert max-w-none prose-p:mb-3 prose-strong:text-white prose-ul:my-2 prose-li:mb-1 marker:text-indigo-300" dangerouslySetInnerHTML={{ __html: aiInsight }} />
//             </div>
//           )}
//         </div>
//       </div>
...
//                   <YAxis axisLine={false} tickLine={false} tick={{ fill: "#6b7280", fontSize: 11 }} width={110} tickFormatter={(val) => formatRupiah(val)} />
//                   <Tooltip cursor={{ fill: "#f9fafb" }} contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)" }} formatter={formatRupiahTooltip} />
//                   <Bar dataKey="total" radius={[4, 4, 0, 0]} maxBarSize={40}>
//                     {revenueData.map((_entry, index) => (
//                       <Cell key={`cell-${index}`} fill={DEFAULT_CHART_COLORS[index % DEFAULT_CHART_COLORS.length]} />
//                     ))}
...
//               </ResponsiveContainer>
//             ) : (
//               <div className="flex items-center justify-center w-full h-full text-sm italic text-gray-400">Belum ada data transaksi dalam 6 bulan terakhir.</div>
//             )}
//           </div>
//         </div>
...
//               <ResponsiveContainer width="100%" height="100%" debounce={300}>
//                 <PieChart>
//                   <Pie data={popularProducts} cx="50%" cy="40%" innerRadius={55} outerRadius={85} paddingAngle={3} dataKey="value" stroke="none" style={{ outline: "none" }}>
//                     {popularProducts.map((entry, index) => (
//                       <Cell key={`cell-${index}`} fill={getColorFromName(entry.name, index)} className="transition-opacity duration-300 hover:opacity-80" />
//                     ))}
//                   </Pie>
//                   <Tooltip contentStyle={{ borderRadius: "8px", border: "none", boxShadow: "0 10px 15px -3px rgba(0, 0, 0, 0.1)" }} formatter={(value: any) => [`${value} Unit`, "Terjual"]} />
//                   <Legend verticalAlign="bottom" content={renderCustomLegend} />
//                 </PieChart>
//               </ResponsiveContainer>
//             ) : (
//               <div className="flex items-center justify-center w-full h-full text-sm italic text-gray-400">Belum ada data penjualan produk.</div>
//             )}
//           </div>
//         </div>
//       </div>
...
//                       <p className="text-xs font-bold text-gray-800 truncate w-36" title={prod.name}>{prod.name}</p>
//                     </div>
//                     <span className="px-2 py-1 text-[10px] font-bold text-red-600 rounded-full bg-red-50">{prod.total_returned}x Retur</span>
//                   </div>
//                 ))}
//               </div>
//             ) : (
...
//                   <Line type="monotone" dataKey="orders" name="Total Order" stroke="#8b5cf6" strokeWidth={3} dot={false} activeDot={{ r: 6, fill: "#8b5cf6", stroke: "#fff", strokeWidth: 2 }} />
//                 </LineChart>
//               </ResponsiveContainer>
//             ) : (
//               <div className="flex items-center justify-center w-full h-full text-sm italic text-gray-400">Belum ada data jam pesanan.</div>
...
//       <div className={`grid grid-cols-1 gap-8 lg:grid-cols-3 transition-all duration-700 delay-[800ms] ${isDataReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}>
//         <div className="lg:col-span-1">
//           <ABTestWidget />
//         </div>
//         <div className="lg:col-span-2">
...
export default function AdminDashboard() {
  ...
  // 👇 [BARU] FUNGSI MENGUNDUH FILE DARI API 👇
  const handleExportFinancial = async () => {
    try {
      ...
      // Ubah stream data menjadi Blob (Binary Large Object)
      const blob = await res.blob();

      // Buat URL sementara di browser untuk memicu download
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `Laporan_Keuangan_Gycora_${new Date().toISOString().slice(0, 10)}.csv`;
      document.body.appendChild(a);
      a.click(); // Klik paksa via JS
      a.remove();
      window.URL.revokeObjectURL(url); // Hapus URL untuk hemat memori

      Swal.fire("Berhasil", "Laporan keuangan berhasil diunduh.", "success");
    } catch (error: any) {
      ...
      <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            ...
            className="flex justify-center items-center gap-2 px-5 py-2.5 rounded-xl text-[10px] md:text-sm font-bold shadow-sm transition-all bg-indigo-50 text-indigo-700 border border-indigo-200 hover:bg-indigo-100"
          >
          ...
          <button
            onClick={handleToggleMaintenance}
            className={`flex justify-center items-center gap-3 px-5 py-2.5 rounded-xl text-[10px] md:text-sm font-bold shadow-sm transition-all ${isMaintenance ? 'bg-red-50 text-red-600 border border-red-200 animate-pulse' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}
          >
            <div className="relative flex items-center justify-center w-3 h-3">
              {isMaintenance && <span className="absolute inline-flex w-full h-full bg-red-400 rounded-full opacity-75 animate-ping"></span>}
              <span className={`relative inline-flex rounded-full w-2.5 h-2.5 ${isMaintenance ? 'bg-red-500' : 'bg-emerald-500'}`}></span>
