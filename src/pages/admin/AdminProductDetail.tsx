...
export default function AdminProductDetail() {
  ...
  // 👇 EVALUASI VALIDITAS WAKTU DISKON 👇
          ...
          <div className="p-6 lg:p-8 space-y-8 lg:col-span-2 min-w-0">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4 bg-gray-50 p-4 border border-gray-100 rounded-xl">
                ...
                {/* 👇 TAMPILAN WAKTU DISKON ADMIN 👇 */}
                <div className="flex-1 min-w-[160px] max-w-full flex flex-col p-4 bg-rose-50 border border-rose-100 shadow-sm rounded-xl overflow-x-auto custom-scrollbar">
                  <p className="mb-1 text-[10px] font-bold tracking-widest text-rose-500 uppercase whitespace-nowrap">Diskon Publik</p>
                  {product.discount_price && product.discount_price > 0 ? (
                    <div>
                      <p className="text-base xl:text-lg font-black text-rose-700 whitespace-nowrap">{formatRupiah(product.discount_price)}</p>
                      <div className="flex items-center gap-1.5 mt-1.5">
                        <span className={`px-1.5 py-0.5 text-[8px] font-bold text-white rounded shrink-0 ${isDiscountValid ? 'bg-emerald-500' : 'bg-gray-400'}`}>
                          {isDiscountValid ? 'AKTIF' : 'KEDALUWARSA'}
                        </span>
                        {(product.discount_start_date || product.discount_end_date) && (
                          <p className="text-[9px] font-semibold text-rose-400 whitespace-nowrap">
                            {product.discount_start_date ? new Date(product.discount_start_date).toLocaleDateString('id-ID') : "Skrg"} - {product.discount_end_date ? new Date(product.discount_end_date).toLocaleDateString('id-ID') : "Seterusnya"}
                          </p>
                        )}
                      </div>
                    </div>
                ...
                {/* 👇 TAMPILAN KUOTA HADIAH BUNDLE ADMIN 👇 */}
                <div className="flex-1 min-w-[160px] max-w-full flex flex-col p-4 bg-purple-50 border border-purple-100 shadow-sm rounded-xl overflow-x-auto custom-scrollbar">
                  <p className="mb-1 text-[10px] font-bold tracking-widest text-purple-600 uppercase whitespace-nowrap">Bundle Price</p>
                  {product.is_bundle_active && product.bundle_price && product.bundle_price > 0 ? (
                    <div>
                      <p className="text-base xl:text-lg font-black text-purple-700 whitespace-nowrap">{formatRupiah(product.bundle_price)}</p>
                      {(product.bundle_start_date || product.bundle_end_date) && (
                        <p className="mt-1 text-[10px] font-semibold text-purple-500 leading-tight whitespace-nowrap">
                          {product.bundle_start_date ? new Date(product.bundle_start_date).toLocaleDateString('id-ID') : "Skrg"}
                          {" - "}
                      ...
                      {product.has_bundle_freebie && (
                        <p className="mt-1.5 text-[9px] font-bold text-purple-700 bg-purple-200/50 px-2 py-1 rounded inline-block">
                          🎁 Free {product.bundle_freebie_name} (Sisa: {product.bundle_freebie_quota})
                        </p>
                      )}
                    </div>
                  ) : (
