...
//   // ==========================================
//   // FITUR BARU: EXPORT TO CSV
//   // ==========================================
//   const exportToCSV = () => {
//     if (logs.length === 0) {
//       return Swal.fire("Perhatian", "Tidak ada data log untuk diekspor.", "info");
//     }

//     const headers = ['ID', 'Timestamp', 'Actor', 'Role', 'Action', 'Module', 'Module ID', 'IP Address'];
    
//     const csvRows = logs.map(log => [
//       log.id,
//       new Date(log.created_at).toLocaleString('id-ID').replace(/,/g, ''), // Hapus koma
//       log.user ? `${log.user.first_name} ${log.user.last_name}` : 'System',
//       log.user?.usertype || 'system',
...
//     link.setAttribute("download", `Gycora_Audit_Logs_${new Date().toISOString().split('T')[0]}.csv`);
...
//     Swal.fire({ 
//       title: "Berhasil", 
//       text: "Data Audit Log berhasil diekspor ke CSV.", 
//       icon: "success", 
//       timer: 1500, 
...
//           <button 
//             onClick={exportToCSV} 
//             className="flex items-center gap-2 p-2 px-4 text-sm font-bold text-gray-700 transition bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-50 hover:text-gycora"
...
export default function AdminAuditLogPage() {
  ...
  // 👇 [PERBAIKAN] State untuk Otorisasi Aksi Audit Logs 👇
            ...
            <tbody className="text-gray-600">
              {paginatedLogs.map((log) => (
                <tr key={log.id} className="transition border-b border-gray-50 hover:bg-gray-50/80">
                  ...
                  <td className="p-4 text-right">
                    {/* 👇 BUNGKUS TOMBOL VIEW PAYLOAD DENGAN RBAC 👇 */}
                    {canAccess('audit_logs.detail') ? (
                      <button
                        onClick={() => setSelectedLog(log)}
                        className="text-xs font-bold text-gycora hover:underline"
                      >
                      ...
                      <span className="text-[10px] italic text-gray-400">No Access</span>
                    )}
                  </td>
                </tr>
              ))}
