...
export default function AdminProductStock() {
  ...
  // State untuk Otorisasi CRUD Stok
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [adminUser, setAdminUser] = useState<any>(null);
  const [allowedModules, setAllowedModules] = useState<string[]>([]);
  ...
  const fetchStocks = async () => {
    try {
      const token = localStorage.getItem("admin_token");
      // Asumsi: Endpoint default ini mengambil produk aktif
      const res = await fetch(`${BASE_URL}/api/admin/product-stocks?status=active`, {
        headers: { "Authorization": `Bearer ${token}` }
      });
      const data = await res.json();
      const responseData = data.data ? data.data : data;
  ...
  // 👇 [FITUR BARU] Hitung Statistik Stok Cerdas 👇
  const stats = useMemo(() => {
  ...
  return (
    <div className="p-8 mx-auto space-y-6 font-sans max-w-7xl animate-fade-in-up">
      <div className="flex items-center justify-between p-6 bg-white border border-gray-100 shadow-sm rounded-xl">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Manajemen Stok (Aktif)</h1>
          <p className="mt-1 text-sm text-gray-500">Kelola pergerakan masuk stok barang aktif di Gudang Gycora.</p>
        </div>
        <div className="flex gap-3">
          <Link
            to="/admin/product-stocks/inactive"
      ...
      {/* 👇 [FITUR BARU] Tampilan 4 Kartu Statistik Stok 👇 */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 md:gap-6">
        ...
        <div className="flex items-center gap-4 p-5 bg-white border border-gray-100 shadow-sm rounded-xl">
          ...
          <div>
            <p className="text-[10px] font-bold tracking-widest text-gray-500 uppercase">Total Produk (Aktif)</p>
        ...
        <div className="flex items-center gap-4 p-5 bg-white border border-gray-100 shadow-sm rounded-xl">
          ...
          <div>
            ...
            <h3 className="text-2xl font-black text-amber-600">{loading ? '-' : stats.lowStock}</h3>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 p-5 bg-white border border-gray-100 shadow-sm rounded-xl">
          ...
          <div>
            ...
            <h3 className="text-2xl font-black text-red-600">{loading ? '-' : stats.outOfStock}</h3>
          </div>
        </div>
      </div>
      {/* 👆 ========================================================= 👆 */}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                products.map(p => (
                  <tr key={p.id} className={`transition-colors hover:bg-gray-50 group ${p.stock <= 0 ? 'bg-red-50/30' : ''}`}>
                    <td className="p-4">
                      <p className="text-sm font-bold text-gray-900">{p.name}</p>
                      <p className="mt-0.5 font-mono text-xs text-gray-500">{p.sku}</p>
                    </td>
                    <td className="p-4 text-center">
                      <span className={`text-lg font-black ${p.stock < 10 ? 'text-red-500' : 'text-[#006A4E]'}`}>
                        {p.stock}
                      </span>
                    </td>
                    <td className="p-4">
                      {p.stocks && p.stocks.length > 0 ? (
                        <div className="flex flex-wrap max-w-xs gap-2">
                          {p.stocks.map(batch => (
                            <div key={batch.id} className="bg-gray-100 border border-gray-200 px-2 py-1 rounded text-[10px]" title={`Tanggal: ${new Date(batch.created_at).toLocaleDateString('id-ID')}`}>
                              <span className="font-bold text-gray-700">{batch.batch_code}</span>
                              <span className="ml-1 font-black text-[#006A4E]">({batch.quantity})</span>
                            </div>
                          ))}
                        </div>
                      ) : (
