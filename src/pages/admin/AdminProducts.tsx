...
export default function AdminProducts() {
  ...
  const stats = useMemo(() => {
    ...
    products.forEach((p) => {
      ...
      if (p.discount_start_date && p.discount_start_date !== "0000-00-00 00:00:00") {
        const start = new Date(p.discount_start_date.replace(" ", "T")).getTime();
        if (!isNaN(start) && now < start) isDiscountValid = false;
      }

      if (isDiscountValid && p.discount_end_date && p.discount_end_date !== "0000-00-00 00:00:00") {
        const end = new Date(p.discount_end_date.replace(" ", "T")).getTime();
        if (!isNaN(end) && now > end) isDiscountValid = false;
      }

      if (
        (isDiscountValid && p.discount_price && p.discount_price > 0) ||
        (p.wholesale_price && p.wholesale_price > 0) ||
        (p.voucher_discount_price && p.voucher_discount_price > 0)
      ) {
        onSale++;
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                products.map(p => {
                  // Tambahan logic cek waktu diskon untuk tampilan coret harga di tabel
                  let isDiscountValidForTable = true;
                  const now = Date.now();
                  if (p.discount_start_date && p.discount_start_date !== "0000-00-00 00:00:00") {
                    const start = new Date(p.discount_start_date.replace(" ", "T")).getTime();
                    if (!isNaN(start) && now < start) isDiscountValidForTable = false;
                  }
                  if (isDiscountValidForTable && p.discount_end_date && p.discount_end_date !== "0000-00-00 00:00:00") {
                    const end = new Date(p.discount_end_date.replace(" ", "T")).getTime();
                    if (!isNaN(end) && now > end) isDiscountValidForTable = false;
                  }
                  ...
                  return (
                    <tr key={p.id} className="transition-colors hover:bg-gray-50 group">
                      ...
                      <td className="p-4 text-sm whitespace-nowrap">
                        {isDiscountValidForTable && p.discount_price && p.discount_price > 0 ? (
                          <div className="flex flex-col">
                            <span className="font-bold text-red-600">{formatPrice(p.discount_price)}</span>
                            <span className="text-[10px] text-gray-400 line-through">{formatPrice(p.price)}</span>
                          </div>
