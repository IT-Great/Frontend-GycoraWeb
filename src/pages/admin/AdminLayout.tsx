...
//           <Link
//             to="/admin/events"
//             title={!isSidebarOpen ? "Event" : ""}
//             className={`flex items-center gap-3 py-2.5 rounded-lg font-medium group transition-colors ${pathname.includes("/admin/events") ? "bg-gycora-light text-gycora-dark" : "text-gray-700 hover:bg-gray-100"} ${isSidebarOpen ? "px-4" : "justify-center px-0"}`}
//           >
//             <svg
...
//               />
//             </svg>
//             {isSidebarOpen && <span className="truncate">Events</span>}
//           </Link>
...
// // PERBAIKAN 1: Pastikan path ini benar
// import { useMessage } from "../../context/MessageContext";
...
// }) {
//   const [isSidebarOpen, setIsSidebarOpen] = useState(true);
//   const [isMobileSidebarOpen, setIsMobileSidebarOpen] = useState(false); // [BARU] State khusus Mobile
//   const [isAuthorized, setIsAuthorized] = useState(false);
...
//   // [BARU] Otomatis tutup sidebar mobile saat rute berpindah
//   useEffect(() => {
...
//       {/* ============================================================== */}
//       {/* 👇 BARU: BACKDROP OVERLAY KHUSUS MOBILE 👇 */}
//       {/* ============================================================== */}
//       {isMobileSidebarOpen && (
//         <div 
//           className="fixed inset-0 z-40 transition-opacity bg-black/50 backdrop-blur-sm md:hidden"
...
//           <div className="flex items-center gap-4">
//             {/* 👇 [PERBAIKAN] Logika Toggle Berbeda untuk Mobile dan Desktop 👇 */}
//             <button
//               onClick={() => {
...
export default function AdminLayout({
  ...
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [isMobileSidebarOpen, setIsMobileSidebarOpen] = useState(false); // State khusus Mobile
  const [isAuthorized, setIsAuthorized] = useState(false);
  ...
  // [BARU] State untuk menyimpan daftar modul yang diizinkan untuk user aktif
  ...
  useEffect(() => {
    ...
    // [BARU] Mengambil Access Policies dari localStorage
    try {
      const policiesStr = localStorage.getItem("admin_access_policies");
      if (policiesStr) {
        const policies = JSON.parse(policiesStr);
        // Jika bukan superadmin, set array modul yang diizinkan sesuai usertype
  ...
  // [BARU] Helper function untuk mengecek apakah user bisa melihat menu tertentu
  const canAccess = (moduleKey: string) => {
    if (!adminUser) return false;
    if (adminUser.usertype === "superadmin") return true; // Superadmin selalu bisa melihat semua
