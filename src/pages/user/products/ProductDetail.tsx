...
export default function ProductDetail() {
    ...
    const [fomoData, setFomoData] = useState<{ type: 'view' | 'purchase', message: string } | null>(null);
    const [activeViewers, setActiveViewers] = useState<number>(Math.floor(Math.random() * 8) + 2);
    ...
    useEffect(() => {
        ...
        const simInterval = setInterval(() => {
            ...
            if (randomEvent > 0.6) {
                const newViewers = activeViewers + (Math.floor(Math.random() * 5) - 2);
                const finalViewers = Math.max(3, Math.min(newViewers, 45));
                setActiveViewers(finalViewers);
            ...
            else if (randomEvent > 0.92 && !fomoData) {
                const cities = ['Jakarta', 'Surabaya', 'Bandung', 'Medan', 'Makassar', 'Bali', 'Semarang', 'Yogyakarta'];
                const names = ['Budi', 'Siti', 'Andi', 'Rina', 'Dewi', 'Reza', 'Putri', 'Kevin'];
                const c = cities[Math.floor(Math.random() * cities.length)];
                const n = names[Math.floor(Math.random() * names.length)];
                            ...
                            <div className="mt-2 mb-4">
                                {product.bundle_freebie_quota > 0 ? (
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg animate-pulse">
                                        <span>🎁 Free {product.bundle_freebie_name}</span>
                                        <span className="px-1.5 py-0.5 text-[10px] text-white bg-red-500 rounded">
                                            Sisa Kuota: {product.bundle_freebie_quota} Pembeli!
                                        </span>
                                    </span>
                                ) : (
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-gray-400 bg-gray-50 border border-gray-200 rounded-lg line-through">
                                        🎁 Free {product.bundle_freebie_name} (Sudah Habis)
                                    </span>
                                )}
                            </div>
                        )}
