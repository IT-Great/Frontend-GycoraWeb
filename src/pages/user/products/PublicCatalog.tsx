...
//                   // [PERBAIKAN] Lempar juga seluruh data 'products' agar halaman detail bisa langsung memfilter warna!
//                   state={{ 
//                     initialProduct: product,
//                     allProducts: products // Lempar semua data katalog
//                   }}
...
//       })
//       .sort((a, b) => {
//         // [PERBAIKAN] Menambahkan logika pengurutan berdasarkan nama (A-Z)
//         if (sortBy === "name_asc") return a.name.localeCompare(b.name);
//         if (sortBy === "price_asc") return a.price - b.price;
//         if (sortBy === "price_desc") return b.price - a.price;
//         return b.id - a.id; // Default newest (jika sortBy "newest")
//       });
//   }, [products, activeCategory, searchQuery, sortBy]);
...
//   // eslint-disable-next-line @typescript-eslint/no-explicit-any
//   color?: any; // <-- Diubah ke any agar bisa menghandle JSON string maupun array
// }
...
//     // [PERBAIKAN] Validasi Pengecekan Variasi Warna yang lebih aman
...
//               // [PERBAIKAN] Validasi warna untuk label tombol
//               let hasColors = false;
//               if (product.color) {
...
//             {[...Array(8)].map((_, i) => (
//               <div key={i} className="flex flex-col overflow-hidden bg-white border border-gray-100 rounded-2xl">
//                 {/* [PERBAIKAN] Skeleton diubah ke aspect-square */}
...
//                   className="relative flex flex-col overflow-hidden transition-all duration-300 bg-white border border-gray-100 shadow-sm group rounded-2xl hover:shadow-xl hover:border-gycora/30 hover:-translate-y-1"
//                 >
//                   {/* [PERBAIKAN] Penampung gambar diubah menjadi aspect-square */}
...
//               // [BARU] Logika untuk menentukan harga yang ditampilkan dan persentase diskon
//               const isDiscounted = product.discount_price && product.discount_price > 0 && product.discount_price < product.price;
...
//                   className="relative flex flex-col overflow-hidden transition-all duration-300 bg-white border border-gray-100 shadow-sm group rounded-2xl hover:shadow-xl hover:border-gycora/30 hover:-translate-y-1"
//                 >
//                   {/* Penampung gambar aspect-square */}
//                   <div className="relative overflow-hidden aspect-square bg-gray-50">
//                     {product.image_url ? (
//                       <img 
//                         src={product.image_url} 
...
//                         </span>
//                       )}
//                       {/* [BARU] Badge Diskon diletakkan di sudut kiri atas di bawah badge lainnya */}
//                       {isDiscounted && (
//                          <span className="px-2 py-1 text-[10px] font-bold tracking-widest text-white uppercase rounded-md shadow-sm bg-red-600 w-fit">
//                           -{discountPercentage}%
//                         </span>
//                       )}
//                     </div>
...
//               // 👇 [PERBAIKAN] Logika penentuan Harga Akhir berdasarkan Role 👇
...
//               // Jika dia Reseller dan produk punya harga grosir
//               if (isReseller && hasWholesale) {
//                 finalPrice = product.wholesale_price!;
//                 isDiscounted = true; // Kita anggap harga grosir sebagai diskon dari harga normal
//                 discountPercentage = Math.round(((product.price - product.wholesale_price!) / product.price) * 100);
//               } 
//               // Jika dia user biasa/guest dan produk punya diskon publik
...
export default function PublicCatalog() {
  ...
  // Menggunakan global state currency
  const { currency } = useCurrency();
  const [currentCurrency, setCurrentCurrency] = useState<Currency>((currency as Currency) || 'IDR');
  ...
  // ============================================================================
  // [BARU] REAKTIVITAS MULTI-CURRENCY & GLOBAL LOCALSTORAGE INTERCEPTOR
  // ============================================================================
  // const updateCurrencyState = () => {
  //   setCurrentCurrency(localStorage.getItem('currency') || 'IDR');
  // };

  const updateCurrencyState = () => {
    const savedCurrency = localStorage.getItem("currency") as Currency | null;
    setCurrentCurrency(savedCurrency || "IDR");
  };
  ...
  useEffect(() => {
    ...
    if (!(window as any).isLocalStorageOverride) {
      ...
      (window as any).isLocalStorageOverride = true;
    }
    
    updateCurrencyState(); // Init on mount
