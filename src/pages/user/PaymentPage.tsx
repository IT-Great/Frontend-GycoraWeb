...
export default function PaymentPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    city: "", province: "", postal_code: "", location_type: "home", latitude: "", longitude: "", is_default: false,
  });
  ...
  // 👇 STATE UNTUK FLASH SALE WAITING ROOM 👇
  ...
  const formatCurrencyDisplay = useCallback((priceObj: { value: number; curr: string } | null) => {
    if (!priceObj) return "";
    const symbols: any = { USD: "$", SGD: "S$", EUR: "€", AUD: "A$", MYR: "RM", IDR: "Rp " };
    const formatter = new Intl.NumberFormat(priceObj.curr === "IDR" ? "id-ID" : "en-US", { minimumFractionDigits: priceObj.curr === "IDR" ? 0 : 2, maximumFractionDigits: priceObj.curr === "IDR" ? 0 : 2 });
    return `${symbols[priceObj.curr] || priceObj.curr + " "}${formatter.format(priceObj.value)}`;
  }, []);
  ...
  const getActivePriceObj = useCallback((product: any, totalQty: number) => {
    ...
    if (isReseller && dynamicWholesaleObj && dynamicWholesaleObj.value > 0 && totalQty >= 24) return dynamicWholesaleObj;
    else if (dynamicDiscountObj && dynamicDiscountObj.value > 0 && dynamicDiscountObj.value < dynamicPriceObj.value) return dynamicDiscountObj;
    return dynamicPriceObj;
  }, [userType, appliedPromoType, getPriceToDisplay, getDiscountToDisplay, getWholesaleToDisplay]);
  ...
  const checkoutData = useMemo(() => {
    const curr = (currency as Currency) || "IDR";
    let totalValueDisplay = 0; let totalValueIDR = 0; let appliedBundlesCount = 0;
    const itemGrossAmounts: { [cartId: number]: number } = {};
    const isBundledMap: Record<number, boolean> = {};
    const isEligibleForBundleMap: Record<number, boolean> = {};
    let hasBundleProduct = false;
    ...
    checkoutItems.forEach((item) => {
      const prod = item.product; const qty = item.quantity;
      itemGrossAmounts[item.id] = 0; isBundledMap[item.id] = false; isEligibleForBundleMap[item.id] = false;

      const activePriceObj = getActivePriceObj(prod, checkoutCount);
      let normalPriceDisplay = activePriceObj.value;
      if (activePriceObj.curr === "IDR" && curr !== "IDR") normalPriceDisplay = normalPriceDisplay * (exchangeRates?.[curr] || 1);

      let normalPriceIDR = Number(prod.price) || 0;
      const wholesale = Number(prod.wholesale_price) || 0; const discount = Number(prod.discount_price) || 0; const voucher = Number(prod.voucher_discount_price) || 0;

      if (appliedPromoType === "voucher" && voucher > 0) normalPriceIDR = voucher;
      else if (isWholesaleGlobal && wholesale > 0) normalPriceIDR = wholesale;
      else if (discount > 0 && discount < normalPriceIDR) normalPriceIDR = discount;

      if (isWholesaleGlobal && wholesale > 0) {
        itemGrossAmounts[item.id] += normalPriceDisplay * qty; totalValueDisplay += normalPriceDisplay * qty; totalValueIDR += normalPriceIDR * qty; return;
      }

      const sku = (prod.sku || "").toUpperCase(); const isEGB = sku.startsWith("EGB");
      const rawFlag = prod.is_bundle_active; const isBundleActiveFlag = rawFlag === true || rawFlag === 1 || String(rawFlag) === "1" || String(rawFlag).toLowerCase() === "true";
      const catCode = String((prod as any).category?.code || "").toUpperCase(); const isStandaloneBundle = catCode === "BN-01";
      const isBundleValid = isBundleActiveFlag || isStandaloneBundle;
      if (isBundleValid) hasBundleProduct = true;

      let isValidDate = true;
      if (prod.bundle_end_date && prod.bundle_end_date !== "0000-00-00 00:00:00") {
        const d = new Date(prod.bundle_end_date.replace(" ", "T"));
        if (!isNaN(d.getTime())) isValidDate = d.getTime() > Date.now();
      }

      const bundleObjDisplay = getBundleToDisplay(prod); const bundlePriceIDR = Number(prod.bundle_price) || 0;
      const isDriver = isEGB && isBundleValid && isValidDate && bundleObjDisplay && bundleObjDisplay.value > 0;

      if (isDriver) {
        isEligibleForBundleMap[item.id] = true;
        for (let i = 0; i < qty; i++) driversPool.push({ cartId: item.id, normalPrice: normalPriceDisplay, normalIDR: normalPriceIDR, bundlePrice: bundleObjDisplay.value, bundleIDR: bundlePriceIDR });
      } else if (!isEGB && !isStandaloneBundle) {
        isEligibleForBundleMap[item.id] = true;
        for (let i = 0; i < qty; i++) partnersPool.push({ cartId: item.id, normalPrice: normalPriceDisplay, normalIDR: normalPriceIDR });
      } else {
        itemGrossAmounts[item.id] += normalPriceDisplay * qty; totalValueDisplay += normalPriceDisplay * qty; totalValueIDR += normalPriceIDR * qty;
      }
    });

    driversPool.sort((a, b) => b.bundlePrice - a.bundlePrice);
    while (driversPool.length > 0 && partnersPool.length > 0) {
      const driver = driversPool.shift()!; const partner = partnersPool.shift()!;
      const halfDisplay = driver.bundlePrice / 2;
      itemGrossAmounts[driver.cartId] += halfDisplay; itemGrossAmounts[partner.cartId] += halfDisplay;
      isBundledMap[driver.cartId] = true; isBundledMap[partner.cartId] = true;
      totalValueDisplay += driver.bundlePrice; totalValueIDR += driver.bundleIDR; appliedBundlesCount++;
    }

    driversPool.forEach((d) => { itemGrossAmounts[d.cartId] += d.normalPrice; totalValueDisplay += d.normalPrice; totalValueIDR += d.normalIDR; });
    partnersPool.forEach((p) => { itemGrossAmounts[p.cartId] += p.normalPrice; totalValueDisplay += p.normalPrice; totalValueIDR += p.normalIDR; });

    let dynamicDiscountIDR = 0; let freebies: string[] = []; const upsellMessages: { amountNeeded: number; message: string }[] = []; const appliedPromoNames: string[] = [];

    activePromos.forEach((promo) => {
      const rules: PromoRules = typeof promo.rules === "string" ? JSON.parse(promo.rules) : promo.rules;
      let promoDiscount = 0; let currentPromoFreebies: string[] = [];

      if (rules.tiers && Array.isArray(rules.tiers)) {
        const sortedTiers = [...rules.tiers].sort((a, b) => b.min_purchase - a.min_purchase);
        for (const tier of sortedTiers) {
          if (totalValueIDR >= tier.min_purchase) {
            promoDiscount = tier.discount_nominal || 0;
            if (tier.freebies) currentPromoFreebies = [...currentPromoFreebies, ...tier.freebies];
            break;
          }
        }
        if (totalValueIDR > 0) {
          const nextTiers = sortedTiers.filter((t) => totalValueIDR < t.min_purchase).sort((a, b) => a.min_purchase - b.min_purchase);
          if (nextTiers.length > 0) {
            const nextTier = nextTiers[0]; const amountNeeded = nextTier.min_purchase - totalValueIDR; const rewards = [];
            if (nextTier.discount_nominal) rewards.push(`Diskon Rp ${nextTier.discount_nominal / 1000}K`);
            if (nextTier.freebies && nextTier.freebies.length > 0) rewards.push(...nextTier.freebies);
            if (rewards.length > 0) upsellMessages.push({ amountNeeded, message: `${rewards.join(" + ")}! ${promo.banner_badge || ""}` });
          }
        }
      }

      if (hasBundleProduct && rules.bundle_reward && rules.bundle_reward.freebies) currentPromoFreebies = [...currentPromoFreebies, ...rules.bundle_reward.freebies];
      if (promoDiscount > 0 || currentPromoFreebies.length > 0) { dynamicDiscountIDR += promoDiscount; freebies = [...freebies, ...currentPromoFreebies]; appliedPromoNames.push(promo.banner_badge || promo.name); }
    });

    freebies = Array.from(new Set(freebies));
    totalValueIDR -= dynamicDiscountIDR; if (totalValueIDR < 0) totalValueIDR = 0;
    const dynamicDiscountDisplay = convertIDRtoActiveCurrency(dynamicDiscountIDR).value;
    totalValueDisplay -= dynamicDiscountDisplay; if (totalValueDisplay < 0) totalValueDisplay = 0;

    return { totalObj: { value: totalValueDisplay, curr }, totalIDR: totalValueIDR, itemGrossAmounts, isBundledMap, isEligibleForBundleMap, appliedBundlesCount, dynamicDiscountIDR, dynamicDiscountDisplay, freebies, upsellMessages, appliedPromoNames, originalTotalIDR: totalValueIDR + dynamicDiscountIDR };
  }, [checkoutItems, checkoutCount, curr, getActivePriceObj, getBundleToDisplay, appliedPromoType, exchangeRates, userType, getFreshProduct, convertIDRtoActiveCurrency, activePromos]);
  ...
  const actualPromoDiscountIDR = useMemo(() => {
    if (appliedPromoType === "claim") {
      ...
      let shippingCost = 0;
      if (shippingMethod === "biteship" && selectedRate) shippingCost = parseFloat(selectedRate.price);
      return productDiscount + Math.min(10000, shippingCost);
    }
    return promoDiscountAmount;
  }, [appliedPromoType, checkoutTotalIDR, shippingMethod, selectedRate, promoDiscountAmount]);
  ...
  const shippingCostObj = useMemo(() => convertIDRtoActiveCurrency(shippingCostIDR), [shippingCostIDR, convertIDRtoActiveCurrency]);
  ...
  const pollTicketStatus = async (tId: string) => {
    try {
      const token = localStorage.getItem("user_token");
      const res = await fetch(`${BASE_URL}/api/checkout/ticket/${tId}`, {
      ...
      if (data.status === 'success') {
        ...
        (window as any).dataLayer.push({
          ...
          ecommerce: {
            ...
            value: grandTotalObj.value,
            items: checkoutItems.map((item: any) => {
              return { item_id: item.product_id, item_name: item.product.name, price: item.price, quantity: item.quantity };
            }),
          },
        });
        window.location.href = data.checkout_url;
        ...
        setTimeout(() => pollTicketStatus(tId), 2000);
      }
    } catch (err) {
      setTimeout(() => pollTicketStatus(tId), 2000);
    }
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (res.ok && data.ticket_id) {
        ...
        throw new Error(data.message || "Sistem sangat sibuk. Harap coba lagi.");
      }
    } catch (err: any) {
      setIsProcessing(false);
  ...
  const handleGetCurrentLocation = () => { setIsGettingLocation(true); if (navigator.geolocation) { navigator.geolocation.getCurrentPosition((position) => { const lat = position.coords.latitude; const lng = position.coords.longitude; setMapPosition([lat, lng]); fetchAddressFromCoords(lat, lng); setIsGettingLocation(false); }, () => { Swal.fire("Akses Ditolak", t("warn_location_denied"), "warning"); setIsGettingLocation(false); }); } else { Swal.fire("Tidak Mendukung", t("warn_location_unsupported"), "error"); setIsGettingLocation(false); } };
  const fetchAddressFromCoords = async (lat: number, lng: number) => { try { const res = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`); const data = await res.json(); if (data && data.address) { const { address } = data; const newCity = address.city || address.town || address.county || ""; const newRegion = address.suburb || address.village || address.neighbourhood || ""; const newProvince = address.state || ""; const newPostal = address.postcode || ""; const roadName = address.road || ""; const houseNumber = address.house_number || ""; const fullStreet = roadName ? `${roadName} ${houseNumber}`.trim() : data.display_name; setFormData((prev) => ({ ...prev, latitude: lat.toString(), longitude: lng.toString(), address_location: fullStreet, city: newCity, province: newProvince, region: newRegion, postal_code: newPostal, })); } } catch (error) { console.error(error); } };
  const MapEvents = useCallback(() => { useMapEvents({ click(e) { setMapPosition([e.latlng.lat, e.latlng.lng]); fetchAddressFromCoords(e.latlng.lat, e.latlng.lng); }, }); return null; }, []);
  const MapCenterUpdater = ({ position }: { position: [number, number] }) => { const map = useMap(); useEffect(() => { map.setView(position, map.getZoom()); }, [position, map]); return null; };
  useEffect(() => { if (pointsUsed > maxPointsAllowed) { setPointsUsed(maxPointsAllowed); setPointsInput(maxPointsAllowed > 0 ? maxPointsAllowed : ""); } }, [maxPointsAllowed, pointsUsed]);
  const handleOpenModal = (address: Address | null = null) => { if (address) { setEditingId(address.id); const lat = parseFloat(address.details.latitude); const lng = parseFloat(address.details.longitude); setMapPosition(!isNaN(lat) && !isNaN(lng) ? [lat, lng] : defaultPosition); setFormData({ region: address.details.region || "", first_name_address: address.receiver.first_name, last_name_address: address.receiver.last_name, address_location: address.details.address_location, city: address.details.city, province: address.details.province, postal_code: address.details.postal_code, location_type: address.details.type, latitude: address.details.latitude || "", longitude: address.details.longitude || "", is_default: address.is_default, }); } else { setEditingId(null); setMapPosition(defaultPosition); setFormData({ region: "", first_name_address: "", last_name_address: "", address_location: "", city: "", province: "", postal_code: "", location_type: "home", latitude: "", longitude: "", is_default: false, }); } setIsModalOpen(true); };
  const fetchAddresses = async (token: string) => { try { const res = await fetch(`${BASE_URL}/api/addresses`, { headers: { Authorization: `Bearer ${token}`, Accept: "application/json", }, }); if (res.ok) { const data = await res.json(); const addrArray = data.data ? data.data : data; setAddresses(addrArray || []); if (addrArray && addrArray.length > 0) { const defaultAddr = addrArray.find((a: any) => a.is_default); setSelectedAddressId(defaultAddr ? defaultAddr.id : addrArray[0].id); } else { setSelectedAddressId(null); } } } catch (err) { console.error(err); } };
  useEffect(() => { const fetchCatalog = async () => { try { const res = await fetch(`${BASE_URL}/api/products`); if (res.ok) { const data = await res.json(); setCatalogProducts(data.data ? data.data : data); } } catch (err) { console.error(err); } finally { setIsCatalogLoaded(true); } }; fetchCatalog(); }, []);
  useEffect(() => { if (selectedItemIds.length === 0) { navigate(`${urlPrefix}/cart`); return; } const loadData = async () => { const token = localStorage.getItem("user_token"); const userStr = localStorage.getItem("user_data"); if (!token) { navigate(`${urlPrefix}/login`); return; } if (userStr) { const user = JSON.parse(userStr); setAvailablePoints(user.point || 0); setUserType(user.usertype || "user"); } await fetchAddresses(token); const now = new Date(); now.setHours(now.getHours() + 1); setDeliveryDate(now.toISOString().split("T")[0]); setDeliveryTime(`${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`); setIsPageLoading(false); }; loadData(); }, [navigate, selectedItemIds.length]);
  useEffect(() => { if (selectedAddressId && selectedItemIds.length > 0 && shippingMethod === "biteship") { const getRates = async () => { setIsLoadingRates(true); setSelectedRate(null); setRawShippingRates([]); try { const token = localStorage.getItem("user_token"); const res = await fetch(`${BASE_URL}/api/shipping/rates`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}`, Accept: "application/json", }, body: JSON.stringify({ address_id: selectedAddressId, cart_ids: selectedItemIds, }), }); if (res.ok) { const data = await res.json(); if (data.pricing) setRawShippingRates(data.pricing); } } catch (err) { console.error(err); } finally { setIsLoadingRates(false); } }; getRates(); } }, [selectedAddressId, selectedItemIds, shippingMethod]);
  const processedShippingRates = useMemo(() => { if (!rawShippingRates || rawShippingRates.length === 0) return []; return rawShippingRates.map((rate) => ({ ...rate, is_disabled: false, disable_reason: "" })).sort((a, b) => a.is_disabled === b.is_disabled ? 0 : a.is_disabled ? 1 : -1,); }, [rawShippingRates]);
  const applyPromo = async (e: any) => { e.preventDefault(); if (!promoInput) return; setIsVerifyingPromo(true); try { const token = localStorage.getItem("user_token"); const res = await fetch(`${BASE_URL}/api/promo/verify`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}`, Accept: "application/json", }, body: JSON.stringify({ promo_code: promoInput }), }); const data = await res.json(); if (!res.ok) throw new Error(data.message || "Promo tidak valid"); if (checkoutTotalIDR < 50000) throw new Error("Minimum belanja Rp 50.000"); setAppliedPromoCode(promoInput.toUpperCase()); setAppliedPromoType(data.promo_type); if (data.promo_type === "claim") { setPromoDiscountAmount(0); setPromoMessage("✅ " + data.message + " (10% OFF + Subsidi Ongkir 10K)",); } else { setPromoDiscountAmount(data.discount_value); setPromoMessage("✅ " + data.message + " (Harga Khusus Diterapkan)"); } } catch (err: any) { removePromo(); setPromoMessage("❌ " + err.message); } finally { setIsVerifyingPromo(false); } };
  const removePromo = () => { setPromoInput(""); setAppliedPromoCode(null); setAppliedPromoType(null); setPromoDiscountAmount(0); setPromoMessage(""); };
  const handleApplyPoints = (e: any) => { e.preventDefault(); const ptsToUse = Number(pointsInput); if (ptsToUse > availablePoints) return Swal.fire("Peringatan", `Anda hanya memiliki ${availablePoints} poin.`, "warning"); if (ptsToUse > maxPointsAllowed) { setPointsInput(maxPointsAllowed); setPointsUsed(maxPointsAllowed); return; } setPointsUsed(ptsToUse); };
  const handleRemovePoints = () => { setPointsInput(""); setPointsUsed(0); };
  const handleImageError = (company: string) => { setImageErrors((prev) => ({ ...prev, [company]: true })); };
  const getCourierLogo = (company: string) => { if (!company) return null; const map: Record<string, string> = { jne: "jne.png", sicepat: "sicepat.png", jnt: "jnt.png", anteraja: "anteraja.png", gojek: "gojek.png", grab: "grab.png", paxel: "paxel.png", ninja: "ninja.png", }; return map[company.toLowerCase()] ? "/courier_images/" + map[company.toLowerCase()] : null; };
  const handleSubmitAddress = async (e: React.FormEvent) => { e.preventDefault(); if (!formData.latitude || !formData.longitude) return Swal.fire(t("notification"), t("warn_select_location"), "warning"); const token = localStorage.getItem("user_token"); const method = editingId ? "PUT" : "POST"; const url = editingId ? `${BASE_URL}/api/addresses/${editingId}` : `${BASE_URL}/api/addresses`; try { const res = await fetch(url, { method, headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}`, }, body: JSON.stringify(formData), }); if (res.ok) { Swal.fire({ icon: "success", title: "Berhasil!", text: editingId ? t("toast_address_updated") : t("toast_address_added"), timer: 1500, showConfirmButton: false, }); setIsModalOpen(false); fetchAddresses(token!); } else { throw new Error("Gagal menyimpan alamat"); } } catch (error) { console.error(error); Swal.fire(t("error"), t("server_error"), "error"); } };
  const isButtonDisabled = isProcessing || checkoutItems.length === 0 || !selectedAddressId || (shippingMethod === "biteship" && !selectedRate);

  if (isPageLoading || !isCatalogLoaded) return <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-white"><div className="w-8 h-8 border-4 rounded-full border-gycora border-t-transparent animate-spin"></div></div>;

  const buttonStyleClass =
    abVariant === "B"
      ? "bg-red-600 hover:bg-red-700 text-white shadow-red-500/50"
      : "bg-gray-900 hover:bg-black text-white shadow-black/10";

  return (
    <div className="min-h-screen px-6 py-12 mx-auto font-sans md:py-24 max-w-[1440px] animate-fade-in relative">

      {/* ========================================================= */}
      {/* 👇 LAYAR WAITING ROOM (OVERLAY FLASH SALE) 👇 */}
      {/* ========================================================= */}
      {isWaitingRoomOpen && (
        <div className="fixed inset-0 z-[999999] bg-gray-900/95 flex flex-col items-center justify-center px-4 animate-fade-in">
          <div className="bg-black/50 border border-white/10 p-8 md:p-12 rounded-[2rem] text-center max-w-md w-full shadow-2xl backdrop-blur-md">
            <div className="relative w-24 h-24 mx-auto mb-8">
              <div className="absolute inset-0 border-4 border-gray-700 rounded-full"></div>
              <div className="absolute inset-0 border-4 rounded-full border-gycora border-t-transparent animate-spin"></div>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-2xl font-black text-white">🔥</span>
              </div>
            </div>
            <h2 className="text-2xl font-black text-white uppercase tracking-widest mb-2 animate-pulse">Waiting Room</h2>
            <p className="text-emerald-400 font-bold mb-6">Flash Sale Sedang Berlangsung!</p>
            <div className="bg-white/10 border border-white/20 p-6 rounded-2xl mb-6">
              <p className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-1">Posisi Antrean Anda</p>
              <p className="text-5xl font-black text-white">{queuePosition}</p>
            </div>
            <p className="text-sm font-bold text-yellow-400 mb-4 animate-pulse">{queueMessage}</p>
            <p className="text-sm text-gray-400 leading-relaxed">Sistem kami sedang mengamankan stok barang Anda. <br /><span className="text-white font-bold">Harap jangan tutup atau refresh halaman ini.</span></p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col gap-12 lg:flex-row lg:gap-20">
        <div className="flex-grow space-y-12">
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              ...
              {addresses.length > 0 && <button onClick={() => handleOpenModal()} className="text-xs font-bold transition-colors text-emerald-600 hover:text-emerald-800">{t("pay_add_address")}</button>}
            </div>
        ...
        <div className="space-y-6 lg:w-[450px] xl:w-[480px] shrink-0">
          <div className="sticky p-8 border border-gray-100 shadow-xl bg-gray-50 rounded-3xl top-28">
            ...
            {checkoutData.upsellMessages.map((upsell, idx) => (
              <div key={idx} className="p-3 mb-4 text-xs font-medium text-center text-red-700 bg-red-100 border border-red-200 rounded-xl">
                Tambah <span className="font-bold text-red-600">{formatCurrencyDisplay(convertIDRtoActiveCurrency(upsell.amountNeeded))}</span> lagi untuk <strong>{upsell.message}</strong>
              </div>
            ))}

            {checkoutData.freebies.length > 0 && (
              <div className="flex flex-col gap-2 p-4 mb-4 border border-red-200 bg-red-50 rounded-2xl">
                <div className="flex items-center gap-2"><span className="text-base">🎁</span><span className="text-xs font-extrabold tracking-wide text-red-700 uppercase">Promo Rewards!</span></div>
                <ul className="pl-6 space-y-1 text-xs font-bold text-red-600 list-disc">
                  {checkoutData.freebies.map((f: string, i: number) => <li key={i}>{f}</li>)}
                </ul>
              </div>
            )}
            ...
            <div className="space-y-4 mb-8 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              {checkoutItems.map((item: any) => {
                let colorHex = item.color; let colorName = "";
                try {
                  const parsed = JSON.parse(item.color as string);
                  if (parsed.hex) { colorHex = parsed.hex; colorName = parsed.name || ""; }
                } catch (error) {
                  if (Array.isArray(item.product.color)) {
                    const matched = item.product.color.find((c: any) => (typeof c === "object" && c !== null && c.hex === item.color) || c === item.color);
                    if (matched && typeof matched === "object" && matched !== null) colorName = (matched as { name?: string }).name || "";
                  }
                }
                const freshProd = getFreshProduct(item.product);
                const activePriceObj = getActivePriceObj(freshProd, checkoutCount);
                ...
                const isWholesaleActive = userType === "reseller" && Number(freshProd.wholesale_price) > 0 && checkoutCount >= 24;
                const isBundled = checkoutData.isBundledMap[item.id];
                const catCode = String(freshProd.category?.code || "").toUpperCase();
                const isStandaloneBundle = catCode === "BN-01";
                const calculatedGross = checkoutData.itemGrossAmounts[item.id];
                const currentGrossAmountObj = { value: calculatedGross !== undefined ? calculatedGross : activePriceObj.value * item.quantity, curr: checkoutData.totalObj.curr };
                ...
                return (
                  <div key={item.id} className="flex gap-4">
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="w-40 text-[11px] font-bold text-gray-900 uppercase truncate" title={freshProd.name}>{freshProd.name}</p>
                        {isWholesaleActive && <span className="px-1.5 py-0.5 text-[8px] font-bold text-white bg-blue-600 rounded shrink-0">GROSIR</span>}
                        {(isBundled || isStandaloneBundle) && <span className="px-1.5 py-0.5 text-[8px] font-bold text-purple-600 bg-purple-100 rounded uppercase shrink-0">Bundle</span>}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5">
                        <p className="text-[10px] text-gray-400">Qty: {item.quantity}</p>
                        {item.color && (
                          <><span className="w-1 h-1 bg-gray-300 rounded-full"></span>
                            <div className="flex items-center gap-1.5">
                              <div className="w-3 h-3 border border-gray-300 rounded-full shadow-sm shrink-0" style={{ backgroundColor: colorHex }}></div>
                              {colorName && <span className="text-[10px] font-bold text-gray-500 uppercase truncate max-w-[80px]">{colorName}</span>}
                            </div>
                          </>
                        )}
                      </div>
                      <p className={`mt-1 text-xs font-medium ${isBundled || isStandaloneBundle ? "text-purple-600" : isWholesaleActive ? "text-blue-600" : "text-gycora"}`}>
                        {formatCurrencyDisplay(currentGrossAmountObj)} {isDiscounted && !isBundled && !isStandaloneBundle && (<span className="text-[9px] line-through text-gray-400 ml-1">{formatCurrencyDisplay(originalGrossAmountObj)}</span>)}
                      </p>
                    </div>
                  </div>
                );
            ...
            <div className="pt-4 space-y-3 text-sm border-t border-gray-200">
              <div className="flex justify-between text-gray-500"><span>{t("pay_total_items")}</span><span className="font-bold text-gray-900">{checkoutCount} items</span></div>
              <div className="flex justify-between text-gray-500"><span>{t("pay_product_subtotal")}</span><span className={appliedPromoType === "voucher" ? "text-amber-600 font-bold" : ""}>{formatCurrencyDisplay(checkoutData.totalObj)}</span></div>

              {checkoutData.dynamicDiscountDisplay > 0 && (
                <div className="flex justify-between text-sm font-bold text-red-600">
                  <span className="max-w-[150px] truncate" title={checkoutData.appliedPromoNames.join(" + ") || "Promo Active"}>{checkoutData.appliedPromoNames.join(" + ") || "Promo Active"}</span>
                  <span>- {formatCurrencyDisplay({ value: checkoutData.dynamicDiscountDisplay, curr: checkoutData.totalObj.curr })}</span>
                </div>
              )}
              ...
              <button
                ...
                className={`flex items-center justify-center w-full gap-3 py-4 mt-8 text-xs font-bold tracking-[0.3em] uppercase transition-all duration-300 shadow-xl rounded-2xl disabled:bg-gray-300 disabled:shadow-none ${buttonStyleClass}`}
              >
                {!isProcessing ? abVariant === "B" ? "Tuntaskan Pesanan" : "Pay Now" : t("pay_btn_processing")}
              </button>

              <p className="text-[9px] text-center text-gray-400 mt-2 italic">A/B Test Variant: <strong>{abVariant}</strong>.</p>
              {!selectedAddressId && <p className="mt-4 text-[10px] tracking-tighter text-center text-red-500 uppercase">{t("pay_alert_no_address")}</p>}
              {shippingMethod === "biteship" && !selectedRate && <p className="mt-4 text-[10px] tracking-tighter text-center text-red-500 uppercase">{t("pay_alert_no_courier")}</p>}
            </div>
          </div>
