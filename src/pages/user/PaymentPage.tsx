...
export default function PaymentPage() {
  ...
  // 👇 [BARU] State khusus untuk Flash Sale Waiting Room 👇
  const [isProcessing, setIsProcessing] = useState(false);
  ...
  // =========================================================================
  // 🌟 [BARU] FUNGSI HANDLER CHECKOUT (DENGAN WAITING ROOM) 🌟
  // =========================================================================
  const handlePayment = async () => {
    setIsProcessing(true);
    try {
      ...
      // 👇 Cek Jika Mendapat Respons 429 (Melesat ke Waiting Room) 👇
      if (res.status === 429 && data.in_queue) {
        setIsWaitingRoom(true);
        setQueueNumber(data.queue_number);
        
        // Panggil diri sendiri lagi secara otomatis setelah 3 detik
        setTimeout(() => {
          handlePayment();
        }, 3000);
        
        return; // Hentikan proses bawahnya
      }

      if (res.ok && data.checkout_url) {
        setIsWaitingRoom(false); // Matikan modal waiting room jika sukses masuk
        (window as any).dataLayer = (window as any).dataLayer || [];
        (window as any).dataLayer.push({
          event: "add_payment_info",
          ecommerce: {
  ...
  return (
    <div className="min-h-screen px-6 py-12 mx-auto font-sans md:py-24 max-w-[1440px] animate-fade-in relative">
      ...
      {/* ==================================================================== */}
      {/* 🌟 [BARU] MODAL WAITING ROOM (EFEK FOMO SEPERTI WAR TIKET) 🌟 */}
      {/* ==================================================================== */}
      {isWaitingRoom && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
          <div className="flex flex-col items-center justify-center max-w-md p-8 text-center bg-white shadow-2xl rounded-3xl">
            <div className="relative flex items-center justify-center w-20 h-20 mb-6">
              <div className="absolute inset-0 border-4 border-gray-200 rounded-full"></div>
              <div className="absolute inset-0 border-4 border-[#006A4E] rounded-full border-t-transparent animate-spin"></div>
              <svg className="w-8 h-8 text-[#006A4E] animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <h2 className="mb-2 text-2xl font-black tracking-tight text-gray-900 uppercase">Anda Berada di Antrean</h2>
            <p className="mb-6 text-sm font-medium text-gray-500">
              Trafik sedang sangat tinggi! Kami sedang memproses pesanan secara bergilir agar sistem tetap stabil. <br/><strong className="inline-block mt-1 text-red-600">Mohon jangan refresh halaman ini!</strong>
            </p>
            
            <div className="w-full p-4 mb-2 border border-gray-100 bg-gray-50 rounded-2xl">
              <p className="text-[10px] font-bold tracking-widest text-gray-400 uppercase mb-1">Nomor Antrean Anda</p>
              <p className="text-4xl font-black text-[#006A4E] animate-pulse">
                {queueNumber}
              </p>
            </div>
            <p className="text-xs italic text-gray-400">Sistem otomatis mencoba ulang setiap beberapa detik...</p>
          </div>
        </div>
      )}
      {/* ==================================================================== */}
