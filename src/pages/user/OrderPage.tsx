...
export default function OrderPage() {
  ...
  const requestRefund = async (id: number) => {
      ...
      html: `
        <div class="text-left space-y-4">
          ...
          <div>
            ...
            <p class="text-[10px] text-gray-400 mt-1">Max 10MB. Formats: JPG, PNG, MP4.</p>
          </div>
        </div>
      `,
      showCancelButton: true,
    ...
    if (isConfirmed && formValues) {
      ...
      try {
        ...
        Swal.fire("Error", err.message || "Failed to request refund", "error");
      }
    }
  };
  ...
  return (
    <div className="min-h-screen px-6 py-20 mx-auto font-sans max-w-7xl">
      <div className="flex items-center justify-between mb-10">
        <h1 className="font-serif text-4xl tracking-tighter text-gray-900 uppercase">
          Lacak Pesanan
        </h1>
      </div>
      ...
      <div className="flex flex-col items-center justify-between gap-4 mb-8 md:flex-row">
        ...
        <div className="flex items-center w-full gap-2 md:w-auto">
          <span className="text-xs font-bold tracking-wide text-gray-400 uppercase">Show:</span>
          <select
            value={itemsPerPage}
            onChange={(e) => setItemsPerPage(Number(e.target.value))}
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-white border border-gray-100 rounded-2xl animate-fade-in">
          <p className="italic text-gray-400">No orders found matching the selected filters.</p>
          <button onClick={resetFilters} className="inline-block mt-6 text-xs font-bold tracking-widest text-black underline uppercase">
            Clear Filters
          </button>
        </div>
      ) : (
        <div className="space-y-8 animate-fade-in">
          {paginatedTransactions.map((order) => (
            <div key={order.id} className="relative overflow-hidden transition-shadow duration-300 bg-white border border-gray-100 shadow-sm hover:shadow-md rounded-2xl">
              <div className="flex flex-col items-start justify-between gap-4 px-6 py-4 border-b border-gray-100 md:flex-row md:items-center bg-gray-50">
                <div className="flex flex-col w-full gap-4 md:flex-row md:items-center md:gap-8 md:w-auto">
                  <div className="w-full md:w-auto">
                    ...
                    <p className="font-mono text-sm font-bold text-gray-800 truncate">{order.order_id}</p>
                  </div>
                  <div className="w-full md:w-auto">
                    <p className="font-bold text-[10px] text-gray-400 uppercase tracking-[0.2em] mb-1">Date</p>
                    <p className="text-xs font-bold text-gray-800">{formatDateTime(order.created_at)}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end w-full gap-2 md:w-auto">
                  <div className="flex items-center justify-between w-full gap-3 md:justify-end md:w-auto">
                    ...
                    <span className={`px-3 py-1 rounded-full font-bold text-[10px] uppercase tracking-tighter ${statusClass(order.status)}`}>
                      {formatStatus(order.status)}
                    </span>
                  </div>
                  ...
                  {order.shipping_method === 'biteship' ? (
                    <div className="flex items-center justify-between w-full gap-3 md:justify-end md:w-auto">
                      ...
                      <span className={`px-3 py-1 rounded-full font-bold text-[10px] uppercase tracking-tighter border ${shippingStatusClass(order.shipping_status)}`}>
                        {formatStatus(order.shipping_status || "Pending")}
                      </span>
                    </div>
                  ) : order.shipping_method === 'free' ? (
              ...
              <div className="relative flex flex-col gap-6 px-6 py-4 bg-white border-b border-gray-100 md:flex-row md:gap-12">
                {userData?.is_membership && order.point > 0 && order.status === 'completed' && (
                  <div className="static md:absolute md:top-4 md:right-6 bg-gradient-to-r from-yellow-100 to-yellow-50 border border-yellow-200 px-3 py-1.5 rounded-lg flex items-center gap-2 shadow-sm w-max mb-4 md:mb-0">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {order.status === "completed" && (
                          <button
                            ...
                            className="px-2 py-1 mt-1 text-[8px] md:text-[10px] font-bold tracking-widest text-gycora uppercase transition-colors bg-white border border-gycora hover:bg-emerald-50 rounded w-max"
                          >
                            Review
                          </button>
                        )}
              ...
              <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div className="flex flex-col pb-4 mb-4 space-y-1 border-b border-gray-200">
                  ...
                  <div className="flex items-center justify-between pt-2 mt-2 text-sm font-bold text-gray-900 border-t border-gray-200 border-dashed">
                    ...
                    <span className="text-lg text-gycora">{formatPrice(getGrandTotal(order))}</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
                  ...
                  <div className="flex flex-wrap justify-center w-full gap-3 md:justify-end md:w-auto">
                    {canCancel(order.status) && (
                      <button onClick={() => cancelOrder(order.id)} className="hover:bg-red-50 px-6 py-2 border border-red-200 rounded-xl font-bold text-red-600 text-xs uppercase tracking-widest transition flex-1 md:flex-none min-w-[120px] max-w-[200px]">Cancel</button>
                    )}
                    {canPay(order.status) && (
                      <button onClick={() => redirectToPayment(order)} disabled={countdowns[order.id] === 'Expired'} className="bg-black hover:bg-gray-800 disabled:bg-gray-300 disabled:cursor-not-allowed px-6 py-2 rounded-xl font-bold text-white text-xs uppercase tracking-widest transition flex-1 md:flex-none min-w-[120px] max-w-[200px]">Pay Now</button>
                    )}
                    {["processing", "completed", "cancelled", "refund_requested", "refund_approved", "refunded", "refund_rejected", "refund_manual_required", "shipping_failed", "returned"].includes(order.status) && order.shipping_method === 'biteship' && (
                      <button onClick={() => navigate(`/tracking/${order.id}`, { state: { paymentMethod: order.payment_method } })} className="bg-black hover:bg-gray-800 px-6 py-2 rounded-xl font-bold text-white text-xs uppercase tracking-widest transition shadow-sm flex-1 md:flex-none min-w-[120px] max-w-[200px]">Track Order</button>
                    )}
                    {canRequestRefund(order) && (
                      <button onClick={() => requestRefund(order.id)} className="hover:bg-gray-100 px-6 py-2 border border-gray-300 rounded-xl font-bold text-gray-600 text-xs uppercase tracking-widest transition flex-1 md:flex-none min-w-[120px] max-w-[200px]">Refund</button>
                    )}
                    {order.status === 'refund_requested' && (
                      <div className="bg-amber-100 px-4 py-2 rounded-xl text-amber-700 text-xs font-bold w-full md:w-auto text-center flex-1 md:flex-none min-w-[120px] max-w-[200px]">Waiting Admin</div>
                    )}
                    {order.status === 'refund_manual_required' && (
                      <div className="bg-pink-100 px-4 py-2 rounded-xl text-pink-700 text-xs font-bold w-full md:w-auto text-center flex-1 md:flex-none min-w-[120px] max-w-[200px]">Manual Refund</div>
                    )}
                    {order.status === 'refund_approved' && (
                      <button onClick={() => processRefundUser(order.id)} className="bg-blue-600 hover:bg-blue-700 px-6 py-2 rounded-xl font-bold text-white text-xs uppercase tracking-widest transition shadow-sm flex-1 md:flex-none min-w-[120px] max-w-[200px]">Refund Now</button>
                    )}
                    {order.status === 'refund_rejected' && (
                      <div className="text-red-500 text-xs font-bold italic w-full md:w-auto text-center flex-1 md:flex-none min-w-[120px] max-w-[200px]">Refund Rejected</div>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="flex flex-col items-center justify-between gap-4 pt-6 mt-8 border-t border-gray-100 md:flex-row">
            ...
            <div className="flex gap-2">
              <button onClick={() => setCurrentPage((prev) => prev - 1)} disabled={currentPage === 1} className="px-4 py-2 text-sm font-medium transition border rounded-xl hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed">Previous</button>
              <div className="flex gap-1">
                {visiblePages.map((page, index) => (
                  <button
                    key={index}
                    onClick={() => typeof page === "number" ? setCurrentPage(page) : null}
                    disabled={page === "..."}
                    className={`w-10 h-10 rounded-xl font-medium transition flex items-center justify-center text-sm ${currentPage === page ? "bg-black text-white border-black" : "hover:bg-gray-50 border-gray-200"} ${page === "..." ? "cursor-default border-transparent hover:bg-transparent" : "border"}`}
                  >
                    {page}
                  </button>
                ))}
              </div>
              <button onClick={() => setCurrentPage((prev) => prev + 1)} disabled={currentPage === totalPages || totalPages === 0} className="px-4 py-2 text-sm font-medium transition border rounded-xl hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed">Next</button>
            </div>
          </div>
        </div>
      )}
