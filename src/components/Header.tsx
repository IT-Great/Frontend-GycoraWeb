...
export default function Header() {
  ...
  // 👇 STATE UNTUK NOTIFIKASI PENGGUNA 👇
  const [isNotificationOpen, setIsNotificationOpen] = useState(false);
  const notificationRef = useRef<HTMLDivElement>(null);
  const [notifications, setNotifications] = useState<any[]>([]);
  const unreadNotifCount = notifications.filter(n => !n.is_read).length;
  ...
  const markAllNotificationsAsRead = async () => {
    try {
      ...
      console.error("Gagal update semua notif", error);
    }
  };
  ...
  useEffect(() => {
    if (isMobileMenuOpen || isSearchOpen || isAllNotifModalOpen) {
      document.body.style.overflow = "hidden";
    } else {
      document.body.style.overflow = "unset";
    }
    return () => { document.body.style.overflow = "unset"; };
  }, [isMobileMenuOpen, isSearchOpen, isAllNotifModalOpen]);
  ...
  return (
    <>
                ...
                <div className="relative" ref={notificationRef}>
                  ...
                  {isNotificationOpen && (
                    <div className="absolute right-0 md:-right-20 z-50 w-80 mt-3 bg-white border border-gray-100 shadow-2xl rounded-2xl overflow-hidden animate-fade-in-up">
                      ...
                      {notifications.length > 0 && (
                        <div 
                          className="p-3 bg-gray-50 text-center border-t border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors"
                          onClick={() => {
                            setIsNotificationOpen(false);
                            setIsAllNotifModalOpen(true);
                          }}
                        >
                          <span className="text-xs font-bold text-gycora uppercase tracking-widest">View All</span>
                        </div>
                      )}
                    </div>
                  )}
      ...
      {isAllNotifModalOpen && (
        <div className="fixed inset-0 z-[200] flex justify-center items-center bg-black/60 backdrop-blur-sm animate-fade-in p-4">
          <div className="flex flex-col overflow-hidden bg-white shadow-2xl w-full max-w-2xl rounded-2xl max-h-[85vh] animate-fade-in-up">
              ...
              <div className="flex items-center gap-3">
                {unreadNotifCount > 0 && (
                  <button 
                    ...
                    className="px-3 py-1.5 text-xs font-bold text-gycora bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-colors"
                  >
                    Tandai Semua Dibaca
                  </button>
                )}
                <button onClick={() => setIsAllNotifModalOpen(false)} className="p-1.5 text-gray-400 bg-white border border-gray-200 rounded-full hover:text-gray-900 hover:bg-gray-100">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 p-2 overflow-y-auto custom-scrollbar">
              {notifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-gray-400 opacity-60 p-10">
                ...
                <div className="space-y-2">
                  {notifications.map((notif) => (
                    <div 
                      key={notif.id}
                      className={`flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 border rounded-xl transition-colors ${!notif.is_read ? "bg-emerald-50/30 border-emerald-100" : "bg-white border-gray-50 hover:bg-gray-50"}`}
                    >
                      <div 
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <h4 className={`text-sm ${!notif.is_read ? 'font-bold text-gray-900' : 'font-medium text-gray-700'}`}>{notif.title}</h4>
                          {!notif.is_read && <span className="px-2 py-0.5 text-[9px] font-bold text-white bg-gycora rounded shadow-sm">Baru</span>}
                        </div>
                        <p className="text-sm text-gray-500 mt-1 line-clamp-2">{notif.message}</p>
                        <p className="text-[10px] text-gray-400 mt-2 font-mono">{new Date(notif.created_at).toLocaleString('id-ID')}</p>
                      </div>
                      ...
                      <div className="shrink-0 flex items-center justify-end w-full md:w-auto mt-2 md:mt-0">
                        {!notif.is_read ? (
                          <button 
                            ...
                            className="text-[10px] font-bold tracking-widest uppercase text-gycora border border-gycora/30 bg-white px-3 py-1.5 rounded-lg hover:bg-gycora hover:text-white transition-colors"
                          >
                            Tandai Dibaca
                          </button>
                        ) : (
